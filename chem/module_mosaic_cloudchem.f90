...
93010	format( a, 8(1x,i6) )
	...
	gas(ngch3o2 ) = rbox(p_ch3o2 ) * factgas
     if(p_c_o2 > param_first_scalar )  gas(ngch3o2 ) = rbox(p_c_o2 ) * factgas 

	gas(ngch3oh ) = rbox(p_ch3oh ) * factgas
      if(p_meoh > param_first_scalar )  gas(ngch3oh ) = rbox(p_meoh ) * factgas 

	gas(ngch3o2h) = rbox(p_op1   ) * factgas
       if(p_cooh > param_first_scalar ) gas(ngch3o2h) = rbox(p_cooh   ) * factgas 
	...
	rbox(p_ora1  ) = gas(nghcooh ) / factgas
     if(p_hcooh .gt. param_first_scalar)   rbox(p_hcooh  ) = gas(nghcooh ) / factgas 
	...
	rbox(p_ch3o2 ) = gas(ngch3o2 ) / factgas
     if(p_c_o2 .gt. param_first_scalar)    rbox(p_c_o2 ) = gas(ngch3o2 ) / factgas 

	rbox(p_ch3oh ) = gas(ngch3oh ) / factgas
      if(p_meoh .gt. param_first_scalar)    rbox(p_meoh ) = gas(ngch3oh ) / factgas 

	rbox(p_op1   ) = gas(ngch3o2h) / factgas
       if(p_cooh .gt. param_first_scalar)  rbox(p_cooh   ) = gas(ngch3o2h) / factgas 
	...
	if (p_ch3o2 .le. numgas_aqfrac)   &
        ...
        if (p_c_o2 .le. numgas_aqfrac .and. p_c_o2 .gt. param_first_scalar)   &
                gas_aqfrac_box(p_c_o2 ) = gas_aqfrac_cmu(ngch3o2 )
	...
	if (p_op1   .le. numgas_aqfrac)   &
        ...
        if (p_cooh   .le. numgas_aqfrac .and. p_cooh .gt. param_first_scalar)   &
                gas_aqfrac_box(p_cooh   ) = gas_aqfrac_cmu(ngch3o2h) 
	...
	return
      ...
      real (kind=8) :: iepoxinit,osinit,tetrolinit 
      real(kind=8) :: iepoxaer,iepoxosaer,tetrolaer
      real (kind=8) :: os_begin,nh4_begin 
      ...
      character(len=200) :: str
       ...
       cair_mol_m3 = cairclm(kpeg)*1.e6     
       conv1a = cair_mol_m3*1.d9         
       conv1b=1.0d0/conv1a

       if(p_iepox > param_first_scalar ) &
         gas_iepox=rbox(p_iepox)*conv1a    

       do lyyy=1,nyyy
        aer(lyyy)=rbulk_cwaer(lyyy,1)*conv1a  
       enddo        

        factlwc=mw_h2o*cair_mol_m3
        water = rcldwtr_sub(kpeg,mpeg)*factlwc*1.0e-3 


        if (ph_cmuaq_cur.ne.0.0d0) then
        mh=10.0d0**(-ph_cmuaq_cur)   
        else
        mh=0.0d0
        endif


       g_iepox_in=gas_iepox 
       delta_g_iepox=0.0d0
       cbar_iepox=0.0d0
       ...
       cbar_iepox = (8.0 *8.314 * tempk / 3.141593 / 0.118)**0.5 * 100 
      ...
      if (qndrop.gt.0.0.and.qcldwtr.ge.1.0e-10) then 
      dropdia=(qcldwtr/qndrop*1.0/1000.0*6.0/3.141593)**(1.0/3.0) 
      dropdia=dropdia*100.0 
      endif
      ...
      droparea=3.141593*dropdia*dropdia*qndrop*rhoair*1.0e-6 
      ...
      nh4_begin=max(aer(l_nh4_aqyy),0.0d0)
          ...
          dt1 = 1.0d0 
          niter = int(max(1.0d0,dtstepc/dt1)) 
      ...
      enddo
       ...
       volaq=aer(l_so4_aqyy)*mw_so4/dens_so4 &         
         ...
         if (alft.gt.1.D-35) then
          aqsoa_con(1) = gas_iepox*conv2  
          aqsoa_con(2)=  aer(l_iepox_aqyy)/max(alft,1.D-35)*1.e-12    
        ...
        if(abs(f(2)).gt.tinys) then
          if (f(2).lt.0.0) then 
             ...
             aqsoa_con(2) = max(aqsoa_con(2) + frt*f(2) * dt1,0.0d0)
              ...
              aqsoa_con(2) = aqsoa_con(2) + f(2) * dt1 
          endif
        endif
...
250     continue
       ...
       tempos=0.0d0
       tempos=aer(l_iepoxos_aqyy)-os_begin  
      if (tempos.gt.0.0d0.and.os_begin.gt.0.0d0) then 
        if (aer(l_so4_aqyy).ge. tempos) then 

          aer(l_so4_aqyy)=aer(l_so4_aqyy)-tempos
          aer(l_nh4_aqyy)=aer(l_nh4_aqyy)-tempos

        elseif (aer(l_so4_aqyy).lt. tempos) then 
         aer(l_iepoxos_aqyy)=aer(l_iepoxos_aqyy)-(tempos-aer(l_so4_aqyy)) 
         aer(l_nh4_aqyy)=aer(l_nh4_aqyy)-aer(l_so4_aqyy) 
      ...
      if(aer(l_nh4_aqyy).lt.0.0d0) aer(l_nh4_aqyy)=0.0d0
        ...
        if (tempnh4.gt.0.0d0.and.nh4_begin.gt.0.0d0) then 
         aer(l_iepoxos_aqyy)= aer(l_iepoxos_aqyy) + tempnh4 * 18.0d0/216.0d0 
        end if
       ...
       delta_g_iepox=(gas_iepox-g_iepox_in)/dtstepc 
       ...
       gammaiepoxcld(kpeg,mpeg)=gamma_iepox_cld 
       effdropdia(kpeg,mpeg)=dropdia 
      ...
      if(dropdia.gt.0.0d0.and.qndrop.gt.0.0.and.qcldwtr.ge.1.0e-10) then
        ...
        * (dropdia/2.0)/ (3.0 * cbar_iepox * alpha_iepox) )**(-1.0) 
      else
       kmt=0.0d0
      endif
      ...
      if (qndrop.gt.0.0.and.qcldwtr.ge.1.0d-10) then 
      dropdia=(qcldwtr/qndrop*1.0d0/1000.0d0*6.0d0/3.141593d0)**(1.0d0/3.0d0) 
      dropdia=dropdia*100.0d0 
      endif
      ...
      droparea=3.141593d0*dropdia*dropdia*qndrop*rhoair*1.0d-6 
      ...
      cbar_iepox = (8.0d0 *8.314d0 * tempk / 3.141593d0 / 0.118d0)**0.5d0 * 100.0d0 
      Dg_iepox = 1.9d0 * 0.118d0**(-2.0d0/3.0d0) / 100.0d0 
      alpha_iepox = 0.1d0  
      Da_iepox = 1.0d-9 * 1.0d4 
      ...
      if(dropdia.gt.0.0d0.and.qndrop.gt.0.0d0.and.qcldwtr.ge.1.0d-10) then
        ...
        * (dropdia/2.0d0)/ (3.0d0 * cbar_iepox * alpha_iepox) )**(-1.0d0) 
      else
       kmt=0.0d0
      endif
     ...
     if (gamma_tot_iepox .gt. 0.0d0) then
        khet_iepox = droparea / &
             (dropdia*0.5d0 / Dg_iepox + 4 / (cbar_iepox*gamma_tot_iepox)) 
      else
        khet_iepox = 0.0d0
       end if
