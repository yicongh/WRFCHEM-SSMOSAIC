  ...
  ijde = max(ide, jde)
   ...
   chem_select: SELECT CASE(config_flags%chem_opt)
     ...
     CASE (SAPRC99_MOSAIC_20BIN_VBS2_KPP)
       CALL wrf_debug(15,'calling saprc99_mosaic_20bin_vbs2_kpp from chem_driver')
       haveaer = .false.
     ...
     CASE (SAPRC99_MOSAIC_12BIN_VBS2_KPP)
       CALL wrf_debug(15,'calling saprc99_mosaic_12bin_vbs2_kpp from chem_driver')
       haveaer = .false.
     CASE (CBMZSORG)
      ...
      do j=jps,min(jde-1,jpe)
         do k=kps,kpe
            do i=ips,min(ide-1,ipe)
              ...
              if (isnan(vvel(i,k,j))) then  
                 if ((k == kps) .and. (.not. isnan(vvel(i,k+1,j)))) then
                    vvel(i,k,j) = vvel(i,k+1,j)  
                 else if ((k == kpe) .and. (.not. isnan(vvel(i,k-1,j)))) then
                    vvel(i,k,j) = vvel(i,k-1,j)
                 else if ((k > kps) .and. (k < kpe) .and. (.not. isnan(vvel(i,k-1,j))) .and. (.not. isnan(vvel(i,k+1,j)))) then
                    vvel(i,k,j) = 0.5*(vvel(i,k+1,j)+vvel(i,k-1,j))
                 else
                    vvel(i,k,j) = 0.0
                 endif
              endif
    ...
    if(config_flags%emiss_inpt_opt > 0 .or. config_flags%dust_opt > 0  &
      ...
      call emissions_driver(grid%id,ktau,grid%dt,grid%DX,                                  &
             ...
             grid%ht, grid%refl_10cm, grid%ic_flashrate, grid%cg_flashrate,                 &
              ...
              grid%plume_hgt_agtf,grid%plume_hgt_agef,                                     &
              grid%plume_hgt_agsv,grid%plume_hgt_aggr,                                     &
...
456           CONTINUE
      ...
      if( do_photstep .and. &
         ...
         call optical_driver (grid%id,curr_secs,grid%dt,config_flags,haveaer,         &
              ...
              grid%totoa_a09,grid%totoa_a10,grid%totoa_a11,grid%totoa_a12,            & 
              grid%totoa_a13,grid%totoa_a14,grid%totoa_a15,grid%totoa_a16,            &
              grid%totoa_a17,grid%totoa_a18,grid%totoa_a19,grid%totoa_a20,            &
...
msg )
   ...
   ENDDO
        ...
        if(config_flags%gaschem_onoff>0)then
         ...
         if( config_flags%chem_opt == SAPRC99_MOSAIC_4BIN_VBS2_KPP.or. &
              ...
              config_flags%chem_opt ==SAPRC99_MOSAIC_20BIN_VBS2_AQ_KPP .or. &
              config_flags%chem_opt ==SAPRC99_MOSAIC_20BIN_VBS2_KPP .or. &
              config_flags%chem_opt ==SAPRC99_MOSAIC_12BIN_VBS2_AQ_KPP .or. &
              config_flags%chem_opt == SAPRC99_MOSAIC_4BIN_IEPOX_VBS_AQ_KPP .or. &
              config_flags%chem_opt == SAPRC99_SIMPLESOM_MOSAIC_4BIN_AQ_KPP .or. & 
              config_flags%chem_opt ==SAPRC99_MOSAIC_12BIN_VBS2_KPP ) then
          do k=kts,kte
           do i=its,ite
            do j=jts,jte
   ...
   CALL wrf_debug(15,'calling kpp_mechanism_driver')
         ...
         ALLOCATE( irr_rates(grid%sm31:grid%em31,grid%sm32:grid%em32,grid%sm33:grid%em33,num_irr_diag),stat=astat )
         IF( astat /= 0 ) THEN
	...
	if(config_flags%aerchem_onoff>0)then
        ...
        call aerosols_driver (grid%id,curr_secs,ktau,grid%dt,grid%ktauc,              &
             config_flags,dtstepc,grid%dx,                                            &
              ...
              grid%phw_bin4,                                                          & 
              grid%ph_no2,                                                            &
              grid%gammaaq,grid%gammacld,                                             &
              grid%clddropdia,                                                        &
              grid%betaaq,grid%betacld,grid%coatbin1,                                 &
              grid%coatbin2,grid%coatbin3,grid%coatbin4,                              &
              grid%ion_prod_rate,                                                     &
              grid%JBNRATE, grid%JBCHRATE, grid%JTNRATE, grid%JTCHRATE,               &
              grid%JBNORGRATE, grid%JBCHORGRATE, grid%JTNRICRATE, grid%JRATE,         &
              grid%JBNRATEAC, grid%JBCHRATEAC, grid%JTNRATEAC, grid%JTCHRATEAC,       &
              grid%JBNORGRATEAC, grid%JBCHORGRATEAC, grid%JTNRICRATEAC, grid%JRATEAC, & 
              ids,ide, jds,jde, kds,kde,                                              &
              ims,ime, jms,jme, kms,kme,                                              &
              its,ite,jts,jte,kts,kte                                                 )
