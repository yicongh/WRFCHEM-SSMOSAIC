...
subroutine plumerise(m1,m2,m3,ia,iz,ja,jz,firesize,mean_fct   &
                    ...
                    ,up,vp,wp,theta,pp,dn0,rv,zt_rams,zm_rams,z_plume) 
 ...
 real,dimension(nspecies), intent(in) :: eburn_in
  ...
  real, dimension(nveg_agreg),intent(out):: z_plume
  ...
  i=1
 	 ...
 	 enddo
         do ispc=1,nspecies
              ...
              eburn_out(1,ispc) = eburn_in(ispc)*0.25   
 	 ...
 	 do iveg_ag=1,nveg_agreg
           ...
           if(mean_fct(iveg_ag) < 1.e-6 ) then
           z_plume(iveg_ag)=0.
           cycle
           endif
...
rmaxtime = float(maxtime)
 ...
 DO WHILE (TIME.LE.RMAXTIME)  
    ...
    dt = min(dt,0.8 * 3.14159 * radius(2) &
         / (2.0 * abs(vel_p(2) - vel_e(2))))
