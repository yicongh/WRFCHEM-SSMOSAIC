...
"ERROR: experimental option selected, please contact wrfchemhelp for assistance")
    ...
    CASE (SAPRC99_MOSAIC_4BIN_IEPOX_VBS_AQ_KPP )
       CALL wrf_debug(15,'calling SAPRC99_MOSAIC_4BIN_IEPOX_VBS_AQ_KPP from chem_driver')
    
    CASE (SAPRC99_SIMPLESOM_MOSAIC_4BIN_AQ_KPP) 
       CALL wrf_debug(15,'calling SAPRC99_SIMPLESOM_MOSAIC_4BIN_AQ_KPP from chem_driver')

    CASE (SAPRC99_MOSAIC_20BIN_VBS2_KPP )  
       CALL wrf_debug(15,'calling SAPRC99_MOSAIC_20BIN_VBS2_4BIN from chem_driver')
       call wrf_message("WARNING: SAPRC99_MOSAIC_20BIN_VBS2_4BIN chemistry option is highly experimental and not recommended for use.")
    CASE (SAPRC99_MOSAIC_20BIN_VBS2_AQ_KPP )
       CALL wrf_debug(15,'calling SAPRC99_MOSAIC_20BIN_VBS2_AQ_KPP from chem_driver')
       call wrf_message("WARNING: SAPRC99_MOSAIC_20BIN_VBS2_AQ_KPP chemistry option is highly experimental and not recommended for use.")
    CASE (SAPRC99_MOSAIC_12BIN_VBS2_KPP )
       CALL wrf_debug(15,'calling SAPRC99_MOSAIC_12BIN_VBS2_4BIN from chem_driver')
       call wrf_message("WARNING: SAPRC99_MOSAIC_12BIN_VBS2_4BIN chemistry option is highly experimental and not recommended for use.")
    CASE (SAPRC99_MOSAIC_12BIN_VBS2_AQ_KPP )
       CALL wrf_debug(15,'calling SAPRC99_MOSAIC_12BIN_VBS2_AQ_KPP from chem_driver')
       call wrf_message("WARNING: SAPRC99_MOSAIC_12BIN_VBS2_AQ_KPP chemistry option is highly experimental and not recommended for use.")
...
"WARNING: dust option 2 currently does not function properly and has been disabled.")
   ...
   if ( config_flags%biomass_burn_opt > 0 .or. config_flags%biomass_burn_opt < 4 .or. config_flags%biomass_burn_opt > 5) then
   ...
   if ( config_flags%wetscav_onoff == 1 ) then
     if( .not. is_moz_chm ) then
       if(  ( config_flags%chem_opt >= 8 .AND. config_flags%chem_opt <= 13) .OR.  &
            ...
            ( config_flags%chem_opt == 205).OR.(config_flags%chem_opt == 207) .OR. & 
             ( config_flags%chem_opt == 209).OR.                                   & 
            ( config_flags%chem_opt == 601 .OR. config_flags%chem_opt == 611) .OR. &
              (config_flags%chem_opt == 109) .OR. ( config_flags%chem_opt == 210)) then 
         call wrf_debug( 15, 'Chemics_init: Wet scavenging turned on' )
       else
         call wrf_error_fatal3("<stdin>",359,&
"ERROR: wet scavenging option requires chem_opt = 8 through 13 or 31 to 36 or 41 to 43 or 109 or &
                               131 to 132 or 203 or 205 or 207 or 209 or 210 or 503 or 504 or 601 or 611 to function.") !krb 06/2021 ssm
       endif

       if ( config_flags%mp_physics /= 2 .and. config_flags%mp_physics /= 10 .and. config_flags%mp_physics /= 11  &
            .and. config_flags%mp_physics /= 17 .and. config_flags%mp_physics /= 18 .and. config_flags%mp_physics /= 22 .and. config_flags%mp_physics /=30) then
         call wrf_error_fatal3("<stdin>",366,&
"ERROR: wet scavenging option requires mp_phys = 2 (Lin et al.) or 10 (Morrison) or 11 (CAMMGMP) or 17/18/22 NSSL_2mom or 30 (Fast SBM) to function.")
       endif
     elseif( id == 1 ) then
       if ( config_flags%mp_physics /= 6 .and. config_flags%mp_physics /= 8 .and. config_flags%mp_physics /= 10 .and. config_flags%mp_physics /= 17  &
            .and. config_flags%mp_physics /= 18 .and. config_flags%mp_physics /= 22.and. config_flags%mp_physics /=30) then
         call wrf_error_fatal3("<stdin>",372,&
"ERROR: wet scavenging option for MOZART,MOZCART requires mp_phys = 6 (WSM6) or 8 (Thompson) or 10 (Morrison) .or 17/18/22 (NSSL 2-moment) or 30 (Fast SBM)to function.")
...
"ERROR: cloud chemistry option requires chem_opt = 8 through 13 or 31 to 36 or 41 to 43 or 109 or &
                              131 to 132 or 203 or 205 or 207 or 209 or 210 or 501 to 504 or 601 or 611 to function.")
      endif

       if ( config_flags%mp_physics /= 2 .and. config_flags%mp_physics /= 10 .and. config_flags%mp_physics /= 11  &
            .and. config_flags%mp_physics /= 17 .and. config_flags%mp_physics /= 18 .and. config_flags%mp_physics /= 22 .and.config_flags%mp_physics /= 30 ) then
         call wrf_error_fatal3("<stdin>",406,&
"ERROR: cloud chemistry option requires mp_phys = 2 (Lin et al.) or 10 (Morrison) or 11 (CAMMGMP) or 17/18/22 NSSL_2mom or 30 (Fast SBM) to function.")
       endif
   ...
   if ( config_flags%cu_physics == 10)  then
      if(  config_flags%chem_opt /= 9   .and.  config_flags%chem_opt /= 10  .and. &
           ...
           config_flags%chem_opt /= 209 .and. &
           config_flags%chem_opt /= 210 .and. & 
           config_flags%chem_opt /= 601 .and.  config_flags%chem_opt /= 611 ) then
         call wrf_error_fatal3("<stdin>",424,&
"ERROR: kfcupscheme requires chem_opt = 9, 10, 32, 34, 202, 203, 209, 210, 601 or 611  to function.")
      endif
   endif
...
" Time averaged variables (cu_diag = 1) requires cu_physics = 3 or 5 or 10 or 93")
   endif
   endif
...
CASE(GOCARTRACM_KPP,RACM_KPP,RACMPM_KPP,RACMSORG_KPP,RACM_MIM_KPP,RACM_ESRLSORG_KPP,&
     ...
     CASE (SAPRC99_MOSAIC_4BIN_IEPOX_VBS_AQ_KPP, SAPRC99_SIMPLESOM_MOSAIC_4BIN_AQ_KPP) 
        ...
        if(config_flags%chem_in_opt == 1 ) grid%vbs_nbin(1)=5
          if(config_flags%chem_in_opt == 0 )then
           grid%vbs_nbin(1)=5
           do j=jts,jte
              do k=kts,kte
                 do i=its,ite
                    ...
                    if (p_biog3_o.gt.1)         chem(i,k,j,p_biog3_o)=0.0
                    if (p_biog4_o.gt.1)         chem(i,k,j,p_biog4_o)=0.0
                    if (p_iepox.gt.1)           chem(i,k,j,p_iepox)=0.0
                    if (p_tetrol.gt.1)           chem(i,k,j,p_tetrol)=0.0
                    if (p_cldtetrol.gt.1)           chem(i,k,j,p_cldtetrol)=0.0
                    if (p_gly.gt.1)             chem(i,k,j,p_gly)=0.0
                    ...
                    if (p_biog3_o_a01.gt.1)     chem(i,k,j,p_biog3_o_a01)=0.0
                    if (p_biog4_o_a01.gt.1)     chem(i,k,j,p_biog4_o_a01)=0.0
                    if (p_opcg2_f_o_a01.gt.1)    chem(i,k,j,p_opcg2_f_o_a01)=0.0
                    if (p_opcg3_f_o_a01.gt.1)    chem(i,k,j,p_opcg3_f_o_a01)=0.0
                    if (p_opcg4_f_o_a01.gt.1)    chem(i,k,j,p_opcg4_f_o_a01)=0.0
                    if (p_opcg5_f_o_a01.gt.1)    chem(i,k,j,p_opcg5_f_o_a01)=0.0
                    if (p_opcg6_f_o_a01.gt.1)    chem(i,k,j,p_opcg6_f_o_a01)=0.0
                    if (p_opcg4_b_o_a01.gt.1)    chem(i,k,j,p_opcg4_b_o_a01)=0.0
                    if (p_opcg5_b_o_a01.gt.1)    chem(i,k,j,p_opcg5_b_o_a01)=0.0
                    if (p_opcg6_b_o_a01.gt.1)    chem(i,k,j,p_opcg6_b_o_a01)=0.0
                    if (p_tanv_a01.gt.1)    chem(i,k,j,p_tanv_a01)=0.0
                    if (p_ternv_a01.gt.1)    chem(i,k,j,p_ternv_a01)=0.0
                    if (p_sopnv_a01.gt.1)    chem(i,k,j,p_sopnv_a01)=0.0
                    if (p_seqnv_a01.gt.1)    chem(i,k,j,p_seqnv_a01)=0.0
                    if (p_sianv_a01.gt.1)    chem(i,k,j,p_sianv_a01)=0.0
                    if (p_iepox_a01.gt.1)    chem(i,k,j,p_iepox_a01)=0.0
                    if (p_iepoxos_a01.gt.1)    chem(i,k,j,p_iepoxos_a01)=0.0
                    if (p_tetrol_a01.gt.1)    chem(i,k,j,p_tetrol_a01)=0.0
                    if (p_cldiepoxos_a01.gt.1)    chem(i,k,j,p_cldiepoxos_a01)=0.0
                    if (p_cldtetrol_a01.gt.1)    chem(i,k,j,p_cldtetrol_a01)=0.0
                    if (p_gly_a01.gt.1)    chem(i,k,j,p_gly_a01)=0.0
                    ...
                    if (p_biog3_o_a02.gt.1)     chem(i,k,j,p_biog3_o_a02)=0.0
                    if (p_biog4_o_a02.gt.1)     chem(i,k,j,p_biog4_o_a02)=0.0
                    if (p_opcg2_f_o_a02.gt.1)    chem(i,k,j,p_opcg2_f_o_a02)=0.0
                    if (p_opcg3_f_o_a02.gt.1)    chem(i,k,j,p_opcg3_f_o_a02)=0.0
                    if (p_opcg4_f_o_a02.gt.1)    chem(i,k,j,p_opcg4_f_o_a02)=0.0
                    if (p_opcg5_f_o_a02.gt.1)    chem(i,k,j,p_opcg5_f_o_a02)=0.0
                    if (p_opcg6_f_o_a02.gt.1)    chem(i,k,j,p_opcg6_f_o_a02)=0.0
                    if (p_opcg4_b_o_a02.gt.1)    chem(i,k,j,p_opcg4_b_o_a02)=0.0
                    if (p_opcg5_b_o_a02.gt.1)    chem(i,k,j,p_opcg5_b_o_a02)=0.0
                    if (p_opcg6_b_o_a02.gt.1)    chem(i,k,j,p_opcg6_b_o_a02)=0.0

                    if (p_tanv_a02.gt.1)    chem(i,k,j,p_tanv_a02)=0.0
                    if (p_ternv_a02.gt.1)    chem(i,k,j,p_ternv_a02)=0.0
                    if (p_sopnv_a02.gt.1)    chem(i,k,j,p_sopnv_a02)=0.0
                    if (p_seqnv_a02.gt.1)    chem(i,k,j,p_seqnv_a02)=0.0
                    if (p_sianv_a02.gt.1)    chem(i,k,j,p_sianv_a02)=0.0
                    if (p_iepox_a02.gt.1)    chem(i,k,j,p_iepox_a02)=0.0
                    if (p_iepoxos_a02.gt.1)    chem(i,k,j,p_iepoxos_a02)=0.0
                    if (p_tetrol_a02.gt.1)    chem(i,k,j,p_tetrol_a02)=0.0
                    if (p_cldiepoxos_a02.gt.1)    chem(i,k,j,p_cldiepoxos_a02)=0.0
                    if (p_cldtetrol_a02.gt.1)    chem(i,k,j,p_cldtetrol_a02)=0.0
                    if (p_gly_a02.gt.1)    chem(i,k,j,p_gly_a02)=0.0
                    ...
                    if (p_biog3_o_a03.gt.1)     chem(i,k,j,p_biog3_o_a03)=0.0
                    if (p_biog4_o_a03.gt.1)     chem(i,k,j,p_biog4_o_a03)=0.0
                    if (p_opcg2_f_o_a03.gt.1)    chem(i,k,j,p_opcg2_f_o_a03)=0.0
                    if (p_opcg3_f_o_a03.gt.1)    chem(i,k,j,p_opcg3_f_o_a03)=0.0
                    if (p_opcg4_f_o_a03.gt.1)    chem(i,k,j,p_opcg4_f_o_a03)=0.0
                    if (p_opcg5_f_o_a03.gt.1)    chem(i,k,j,p_opcg5_f_o_a03)=0.0
                    if (p_opcg6_f_o_a03.gt.1)    chem(i,k,j,p_opcg6_f_o_a03)=0.0
                    if (p_opcg4_b_o_a03.gt.1)    chem(i,k,j,p_opcg4_b_o_a03)=0.0
                    if (p_opcg5_b_o_a03.gt.1)    chem(i,k,j,p_opcg5_b_o_a03)=0.0
                    if (p_opcg6_b_o_a03.gt.1)    chem(i,k,j,p_opcg6_b_o_a03)=0.0
                    if (p_tanv_a03.gt.1)    chem(i,k,j,p_tanv_a03)=0.0
                    if (p_ternv_a03.gt.1)    chem(i,k,j,p_ternv_a03)=0.0
                    if (p_sopnv_a03.gt.1)    chem(i,k,j,p_sopnv_a03)=0.0
                    if (p_seqnv_a03.gt.1)    chem(i,k,j,p_seqnv_a03)=0.0
                    if (p_sianv_a03.gt.1)    chem(i,k,j,p_sianv_a03)=0.0
                    if (p_iepox_a03.gt.1)    chem(i,k,j,p_iepox_a03)=0.0
                    if (p_iepoxos_a03.gt.1)    chem(i,k,j,p_iepoxos_a03)=0.0
                    if (p_tetrol_a03.gt.1)    chem(i,k,j,p_tetrol_a03)=0.0
                    if (p_cldiepoxos_a03.gt.1)    chem(i,k,j,p_cldiepoxos_a03)=0.0
                    if (p_cldtetrol_a03.gt.1)    chem(i,k,j,p_cldtetrol_a03)=0.0
                    if (p_gly_a03.gt.1)    chem(i,k,j,p_gly_a03)=0.0
                    ...
                    if (p_biog3_o_a04.gt.1)     chem(i,k,j,p_biog3_o_a04)=0.0
                    if (p_biog4_o_a04.gt.1)     chem(i,k,j,p_biog4_o_a04)=0.0
                    if (p_opcg2_f_o_a04.gt.1)    chem(i,k,j,p_opcg2_f_o_a04)=0.0
                    if (p_opcg3_f_o_a04.gt.1)    chem(i,k,j,p_opcg3_f_o_a04)=0.0
                    if (p_opcg4_f_o_a04.gt.1)    chem(i,k,j,p_opcg4_f_o_a04)=0.0
                    if (p_opcg5_f_o_a04.gt.1)    chem(i,k,j,p_opcg5_f_o_a04)=0.0
                    if (p_opcg6_f_o_a04.gt.1)    chem(i,k,j,p_opcg6_f_o_a04)=0.0
                    if (p_opcg4_b_o_a04.gt.1)    chem(i,k,j,p_opcg4_b_o_a04)=0.0
                    if (p_opcg5_b_o_a04.gt.1)    chem(i,k,j,p_opcg5_b_o_a04)=0.0
                    if (p_opcg6_b_o_a04.gt.1)    chem(i,k,j,p_opcg6_b_o_a04)=0.0

                    if (p_tanv_a04.gt.1)    chem(i,k,j,p_tanv_a04)=0.0
                    if (p_ternv_a04.gt.1)    chem(i,k,j,p_ternv_a04)=0.0
                    if (p_sopnv_a04.gt.1)    chem(i,k,j,p_sopnv_a04)=0.0
                    if (p_seqnv_a04.gt.1)    chem(i,k,j,p_seqnv_a04)=0.0
                    if (p_sianv_a04.gt.1)    chem(i,k,j,p_sianv_a04)=0.0
                    if (p_iepox_a04.gt.1)    chem(i,k,j,p_iepox_a04)=0.0
                    if (p_iepoxos_a04.gt.1)    chem(i,k,j,p_iepoxos_a04)=0.0
                    if (p_tetrol_a04.gt.1)    chem(i,k,j,p_tetrol_a04)=0.0
                    if (p_cldiepoxos_a04.gt.1)    chem(i,k,j,p_cldiepoxos_a04)=0.0
                    if (p_cldtetrol_a04.gt.1)    chem(i,k,j,p_cldtetrol_a04)=0.0
                    if (p_gly_a04.gt.1)    chem(i,k,j,p_gly_a04)=0.0
                    ...
                    if (p_biog3_o_a05.gt.1)     chem(i,k,j,p_biog3_o_a05)=0.0
                    if (p_biog4_o_a05.gt.1)     chem(i,k,j,p_biog4_o_a05)=0.0
                    if (p_opcg2_f_o_a05.gt.1)    chem(i,k,j,p_opcg2_f_o_a05)=0.0
                    if (p_opcg3_f_o_a05.gt.1)    chem(i,k,j,p_opcg3_f_o_a05)=0.0
                    if (p_opcg4_f_o_a05.gt.1)    chem(i,k,j,p_opcg4_f_o_a05)=0.0
                    if (p_opcg5_f_o_a05.gt.1)    chem(i,k,j,p_opcg5_f_o_a05)=0.0
                    if (p_opcg6_f_o_a05.gt.1)    chem(i,k,j,p_opcg6_f_o_a05)=0.0
                    if (p_tanv_a05.gt.1)    chem(i,k,j,p_tanv_a05)=0.0
                    if (p_ternv_a05.gt.1)    chem(i,k,j,p_ternv_a05)=0.0
                    if (p_sopnv_a05.gt.1)    chem(i,k,j,p_sopnv_a05)=0.0
                    if (p_seqnv_a05.gt.1)    chem(i,k,j,p_seqnv_a05)=0.0
                    if (p_sianv_a05.gt.1)    chem(i,k,j,p_sianv_a05)=0.0
                    if (p_iepox_a05.gt.1)    chem(i,k,j,p_iepox_a05)=0.0
                    if (p_iepoxos_a05.gt.1)    chem(i,k,j,p_iepoxos_a05)=0.0
                    if (p_tetrol_a05.gt.1)    chem(i,k,j,p_tetrol_a05)=0.0
                    if (p_gly_a05.gt.1)    chem(i,k,j,p_gly_a05)=0.0
                    ...
                    if (p_biog3_o_a06.gt.1)     chem(i,k,j,p_biog3_o_a06)=0.0
                    if (p_biog4_o_a06.gt.1)     chem(i,k,j,p_biog4_o_a06)=0.0
                    if (p_opcg2_f_o_a06.gt.1)    chem(i,k,j,p_opcg2_f_o_a06)=0.0
                    if (p_opcg3_f_o_a06.gt.1)    chem(i,k,j,p_opcg3_f_o_a06)=0.0
                    if (p_opcg4_f_o_a06.gt.1)    chem(i,k,j,p_opcg4_f_o_a06)=0.0
                    if (p_opcg5_f_o_a06.gt.1)    chem(i,k,j,p_opcg5_f_o_a06)=0.0
                    if (p_opcg6_f_o_a06.gt.1)    chem(i,k,j,p_opcg6_f_o_a06)=0.0
                    if (p_tanv_a06.gt.1)    chem(i,k,j,p_tanv_a06)=0.0
                    if (p_ternv_a06.gt.1)    chem(i,k,j,p_ternv_a06)=0.0
                    if (p_sopnv_a06.gt.1)    chem(i,k,j,p_sopnv_a06)=0.0
                    if (p_seqnv_a06.gt.1)    chem(i,k,j,p_seqnv_a06)=0.0
                    if (p_sianv_a06.gt.1)    chem(i,k,j,p_sianv_a06)=0.0
                    if (p_iepox_a06.gt.1)    chem(i,k,j,p_iepox_a06)=0.0
                    if (p_iepoxos_a06.gt.1)    chem(i,k,j,p_iepoxos_a06)=0.0
                    if (p_tetrol_a06.gt.1)    chem(i,k,j,p_tetrol_a06)=0.0
                    if (p_gly_a06.gt.1)    chem(i,k,j,p_gly_a06)=0.0
                    ...
                    if (p_biog3_o_a07.gt.1)     chem(i,k,j,p_biog3_o_a07)=0.0
                    if (p_biog4_o_a07.gt.1)     chem(i,k,j,p_biog4_o_a07)=0.0
                    if (p_opcg2_f_o_a07.gt.1)    chem(i,k,j,p_opcg2_f_o_a07)=0.0
                    if (p_opcg3_f_o_a07.gt.1)    chem(i,k,j,p_opcg3_f_o_a07)=0.0
                    if (p_opcg4_f_o_a07.gt.1)    chem(i,k,j,p_opcg4_f_o_a07)=0.0
                    if (p_opcg5_f_o_a07.gt.1)    chem(i,k,j,p_opcg5_f_o_a07)=0.0
                    if (p_opcg6_f_o_a07.gt.1)    chem(i,k,j,p_opcg6_f_o_a07)=0.0
                    if (p_tanv_a07.gt.1)    chem(i,k,j,p_tanv_a07)=0.0
                    if (p_ternv_a07.gt.1)    chem(i,k,j,p_ternv_a07)=0.0
                    if (p_sopnv_a07.gt.1)    chem(i,k,j,p_sopnv_a07)=0.0
                    if (p_seqnv_a07.gt.1)    chem(i,k,j,p_seqnv_a07)=0.0
                    if (p_sianv_a07.gt.1)    chem(i,k,j,p_sianv_a07)=0.0
                    if (p_iepox_a07.gt.1)    chem(i,k,j,p_iepox_a07)=0.0
                    if (p_iepoxos_a07.gt.1)    chem(i,k,j,p_iepoxos_a07)=0.0
                    if (p_tetrol_a07.gt.1)    chem(i,k,j,p_tetrol_a07)=0.0
                    if (p_gly_a07.gt.1)    chem(i,k,j,p_gly_a07)=0.0
                    ...
                    if (p_biog3_o_a08.gt.1)     chem(i,k,j,p_biog3_o_a08)=0.0
                    if (p_biog4_o_a08.gt.1)     chem(i,k,j,p_biog4_o_a08)=0.0
                    if (p_opcg2_f_o_a08.gt.1)    chem(i,k,j,p_opcg2_f_o_a08)=0.0
                    if (p_opcg3_f_o_a08.gt.1)    chem(i,k,j,p_opcg3_f_o_a08)=0.0
                    if (p_opcg4_f_o_a08.gt.1)    chem(i,k,j,p_opcg4_f_o_a08)=0.0
                    if (p_opcg5_f_o_a08.gt.1)    chem(i,k,j,p_opcg5_f_o_a08)=0.0
                    if (p_opcg6_f_o_a08.gt.1)    chem(i,k,j,p_opcg6_f_o_a08)=0.0
                    if (p_tanv_a08.gt.1)    chem(i,k,j,p_tanv_a08)=0.0
                    if (p_ternv_a08.gt.1)    chem(i,k,j,p_ternv_a08)=0.0
                    if (p_sopnv_a08.gt.1)    chem(i,k,j,p_sopnv_a08)=0.0
                    if (p_seqnv_a08.gt.1)    chem(i,k,j,p_seqnv_a08)=0.0
                    if (p_sianv_a08.gt.1)    chem(i,k,j,p_sianv_a08)=0.0
                    if (p_iepox_a08.gt.1)    chem(i,k,j,p_iepox_a08)=0.0
                    if (p_iepoxos_a08.gt.1)    chem(i,k,j,p_iepoxos_a08)=0.0
                    if (p_tetrol_a08.gt.1)    chem(i,k,j,p_tetrol_a08)=0.0
                    if (p_gly_a08.gt.1)    chem(i,k,j,p_gly_a08)=0.0
                    ...
                    if (p_ant2_c_cw01.gt.1)      chem(i,k,j,p_ant2_c_cw01)=0.0
                    if (p_ant3_c_cw01.gt.1)      chem(i,k,j,p_ant3_c_cw01)=0.0
                    if (p_ant4_c_cw01.gt.1)      chem(i,k,j,p_ant4_c_cw01)=0.0
                    if (p_biog1_c_cw01.gt.1)     chem(i,k,j,p_biog1_c_cw01)=0.0
                    if (p_biog2_c_cw01.gt.1)     chem(i,k,j,p_biog2_c_cw01)=0.0
                    if (p_biog3_c_cw01.gt.1)     chem(i,k,j,p_biog3_c_cw01)=0.0
                    if (p_biog4_c_cw01.gt.1)     chem(i,k,j,p_biog4_c_cw01)=0.0
                    if (p_biog1_o_cw01.gt.1)     chem(i,k,j,p_biog1_o_cw01)=0.0
                    if (p_biog2_o_cw01.gt.1)     chem(i,k,j,p_biog2_o_cw01)=0.0
                    if (p_biog3_o_cw01.gt.1)     chem(i,k,j,p_biog3_o_cw01)=0.0
                    if (p_biog4_o_cw01.gt.1)     chem(i,k,j,p_biog4_o_cw01)=0.0
                    if (p_opcg2_f_o_cw01.gt.1)   chem(i,k,j,p_opcg2_f_o_cw01)=0.0
                    if (p_opcg3_f_o_cw01.gt.1)   chem(i,k,j,p_opcg3_f_o_cw01)=0.0
                    if (p_opcg4_f_o_cw01.gt.1)   chem(i,k,j,p_opcg4_f_o_cw01)=0.0
                    if (p_opcg5_f_o_cw01.gt.1)   chem(i,k,j,p_opcg5_f_o_cw01)=0.0
                    if (p_opcg6_f_o_cw01.gt.1)   chem(i,k,j,p_opcg6_f_o_cw01)=0.0
                    if (p_opcg4_b_o_cw01.gt.1)   chem(i,k,j,p_opcg4_b_o_cw01)=0.0
                    if (p_opcg5_b_o_cw01.gt.1)   chem(i,k,j,p_opcg5_b_o_cw01)=0.0
                    if (p_opcg6_b_o_cw01.gt.1)   chem(i,k,j,p_opcg6_b_o_cw01)=0.0
                    if (p_tanv_cw01.gt.1)     chem(i,k,j,p_tanv_cw01)=0.0
                    if (p_ternv_cw01.gt.1)    chem(i,k,j,p_ternv_cw01)=0.0
                    if (p_sopnv_cw01.gt.1)    chem(i,k,j,p_sopnv_cw01)=0.0
                    if (p_seqnv_cw01.gt.1)    chem(i,k,j,p_seqnv_cw01)=0.0
                    if (p_sianv_cw01.gt.1)    chem(i,k,j,p_sianv_cw01)=0.0
                    if (p_iepox_cw01.gt.1)    chem(i,k,j,p_iepox_cw01)=0.0
                    if (p_iepoxos_cw01.gt.1)    chem(i,k,j,p_iepoxos_cw01)=0.0
                    if (p_tetrol_cw01.gt.1)    chem(i,k,j,p_tetrol_cw01)=0.0
                    if (p_cldiepoxos_cw01.gt.1)    chem(i,k,j,p_cldiepoxos_cw01)=0.0
                    if (p_cldtetrol_cw01.gt.1)    chem(i,k,j,p_cldtetrol_cw01)=0.0
                    if (p_gly_cw01.gt.1)    chem(i,k,j,p_gly_cw01)=0.0
                    ...
                    if (p_ant2_c_cw02.gt.1)      chem(i,k,j,p_ant2_c_cw02)=0.0
                    if (p_ant3_c_cw02.gt.1)      chem(i,k,j,p_ant3_c_cw02)=0.0
                    if (p_ant4_c_cw02.gt.1)      chem(i,k,j,p_ant4_c_cw02)=0.0
                    if (p_biog1_c_cw02.gt.1)     chem(i,k,j,p_biog1_c_cw02)=0.0
                    if (p_biog2_c_cw02.gt.1)     chem(i,k,j,p_biog2_c_cw02)=0.0
                    if (p_biog3_c_cw02.gt.1)     chem(i,k,j,p_biog3_c_cw02)=0.0
                    if (p_biog4_c_cw02.gt.1)     chem(i,k,j,p_biog4_c_cw02)=0.0
                    if (p_biog1_o_cw02.gt.1)     chem(i,k,j,p_biog1_o_cw02)=0.0
                    if (p_biog2_o_cw02.gt.1)     chem(i,k,j,p_biog2_o_cw02)=0.0
                    if (p_biog3_o_cw02.gt.1)     chem(i,k,j,p_biog3_o_cw02)=0.0
                    if (p_biog4_o_cw02.gt.1)     chem(i,k,j,p_biog4_o_cw02)=0.0
                    if (p_opcg2_f_o_cw02.gt.1)   chem(i,k,j,p_opcg2_f_o_cw02)=0.0
                    if (p_opcg3_f_o_cw02.gt.1)   chem(i,k,j,p_opcg3_f_o_cw02)=0.0
                    if (p_opcg4_f_o_cw02.gt.1)   chem(i,k,j,p_opcg4_f_o_cw02)=0.0
                    if (p_opcg5_f_o_cw02.gt.1)   chem(i,k,j,p_opcg5_f_o_cw02)=0.0
                    if (p_opcg6_f_o_cw02.gt.1)   chem(i,k,j,p_opcg6_f_o_cw02)=0.0
                    if (p_opcg4_b_o_cw02.gt.1)   chem(i,k,j,p_opcg4_b_o_cw02)=0.0
                    if (p_opcg5_b_o_cw02.gt.1)   chem(i,k,j,p_opcg5_b_o_cw02)=0.0
                    if (p_opcg6_b_o_cw02.gt.1)   chem(i,k,j,p_opcg6_b_o_cw02)=0.0

                    if (p_tanv_cw02.gt.1)        chem(i,k,j,p_tanv_cw02)=0.0
                    if (p_ternv_cw02.gt.1)      chem(i,k,j,p_ternv_cw02)=0.0
                    if (p_sopnv_cw02.gt.1)      chem(i,k,j,p_sopnv_cw02)=0.0
                    if (p_seqnv_cw02.gt.1)      chem(i,k,j,p_seqnv_cw02)=0.0
                    if (p_sianv_cw02.gt.1)      chem(i,k,j,p_sianv_cw02)=0.0
                    if (p_iepox_cw02.gt.1)      chem(i,k,j,p_iepox_cw02)=0.0
                    if (p_iepoxos_cw02.gt.1)    chem(i,k,j,p_iepoxos_cw02)=0.0
                    if (p_tetrol_cw02.gt.1)    chem(i,k,j,p_tetrol_cw02)=0.0
                    if (p_cldiepoxos_cw02.gt.1)    chem(i,k,j,p_cldiepoxos_cw02)=0.0
                    if (p_cldtetrol_cw02.gt.1)    chem(i,k,j,p_cldtetrol_cw02)=0.0
                    if (p_gly_cw02.gt.1)       chem(i,k,j,p_gly_cw02)=0.0
                    ...
                    if (p_ant2_c_cw03.gt.1)      chem(i,k,j,p_ant2_c_cw03)=0.0
                    if (p_ant3_c_cw03.gt.1)      chem(i,k,j,p_ant3_c_cw03)=0.0
                    if (p_ant4_c_cw03.gt.1)      chem(i,k,j,p_ant4_c_cw03)=0.0
                    if (p_biog1_c_cw03.gt.1)     chem(i,k,j,p_biog1_c_cw03)=0.0
                    if (p_biog2_c_cw03.gt.1)     chem(i,k,j,p_biog2_c_cw03)=0.0
                    if (p_biog3_c_cw03.gt.1)     chem(i,k,j,p_biog3_c_cw03)=0.0
                    if (p_biog4_c_cw03.gt.1)     chem(i,k,j,p_biog4_c_cw03)=0.0
                    if (p_biog1_o_cw03.gt.1)     chem(i,k,j,p_biog1_o_cw03)=0.0
                    if (p_biog2_o_cw03.gt.1)     chem(i,k,j,p_biog2_o_cw03)=0.0
                    if (p_biog3_o_cw03.gt.1)     chem(i,k,j,p_biog3_o_cw03)=0.0
                    if (p_biog4_o_cw03.gt.1)     chem(i,k,j,p_biog4_o_cw03)=0.0
                    if (p_opcg2_f_o_cw03.gt.1)   chem(i,k,j,p_opcg2_f_o_cw03)=0.0
                    if (p_opcg3_f_o_cw03.gt.1)   chem(i,k,j,p_opcg3_f_o_cw03)=0.0
                    if (p_opcg4_f_o_cw03.gt.1)   chem(i,k,j,p_opcg4_f_o_cw03)=0.0
                    if (p_opcg5_f_o_cw03.gt.1)   chem(i,k,j,p_opcg5_f_o_cw03)=0.0
                    if (p_opcg6_f_o_cw03.gt.1)   chem(i,k,j,p_opcg6_f_o_cw03)=0.0
                    if (p_opcg4_b_o_cw03.gt.1)   chem(i,k,j,p_opcg4_b_o_cw03)=0.0
                    if (p_opcg5_b_o_cw03.gt.1)   chem(i,k,j,p_opcg5_b_o_cw03)=0.0
                    if (p_opcg6_b_o_cw03.gt.1)   chem(i,k,j,p_opcg6_b_o_cw03)=0.0
                    if (p_tanv_cw03.gt.1)        chem(i,k,j,p_tanv_cw03)=0.0
                    if (p_ternv_cw03.gt.1)       chem(i,k,j,p_ternv_cw03)=0.0
                    if (p_sopnv_cw03.gt.1)       chem(i,k,j,p_sopnv_cw03)=0.0
                    if (p_seqnv_cw03.gt.1)       chem(i,k,j,p_seqnv_cw03)=0.0
                    if (p_sianv_cw03.gt.1)       chem(i,k,j,p_sianv_cw03)=0.0
                    if (p_iepox_cw03.gt.1)       chem(i,k,j,p_iepox_cw03)=0.0
                    if (p_iepoxos_cw03.gt.1)    chem(i,k,j,p_iepoxos_cw03)=0.0
                    if (p_tetrol_cw03.gt.1)    chem(i,k,j,p_tetrol_cw03)=0.0
                    if (p_cldiepoxos_cw03.gt.1)    chem(i,k,j,p_cldiepoxos_cw03)=0.0
                    if (p_cldtetrol_cw03.gt.1)    chem(i,k,j,p_cldtetrol_cw03)=0.0
                    if (p_gly_cw03.gt.1)       chem(i,k,j,p_gly_cw03)=0.0
                    ...
                    if (p_ant2_c_cw04.gt.1)      chem(i,k,j,p_ant2_c_cw04)=0.0
                    if (p_ant3_c_cw04.gt.1)      chem(i,k,j,p_ant3_c_cw04)=0.0
                    if (p_ant4_c_cw04.gt.1)      chem(i,k,j,p_ant4_c_cw04)=0.0
                    if (p_biog1_c_cw04.gt.1)     chem(i,k,j,p_biog1_c_cw04)=0.0
                    if (p_biog2_c_cw04.gt.1)     chem(i,k,j,p_biog2_c_cw04)=0.0
                    if (p_biog3_c_cw04.gt.1)     chem(i,k,j,p_biog3_c_cw04)=0.0
                    if (p_biog4_c_cw04.gt.1)     chem(i,k,j,p_biog4_c_cw04)=0.0
                    if (p_biog1_o_cw04.gt.1)     chem(i,k,j,p_biog1_o_cw04)=0.0
                    if (p_biog2_o_cw04.gt.1)     chem(i,k,j,p_biog2_o_cw04)=0.0
                    if (p_biog3_o_cw04.gt.1)     chem(i,k,j,p_biog3_o_cw04)=0.0
                    if (p_biog4_o_cw04.gt.1)     chem(i,k,j,p_biog4_o_cw04)=0.0
                    if (p_opcg2_f_o_cw04.gt.1)   chem(i,k,j,p_opcg2_f_o_cw04)=0.0
                    if (p_opcg3_f_o_cw04.gt.1)   chem(i,k,j,p_opcg3_f_o_cw04)=0.0
                    if (p_opcg4_f_o_cw04.gt.1)   chem(i,k,j,p_opcg4_f_o_cw04)=0.0
                    if (p_opcg5_f_o_cw04.gt.1)   chem(i,k,j,p_opcg5_f_o_cw04)=0.0
                    if (p_opcg6_f_o_cw04.gt.1)   chem(i,k,j,p_opcg6_f_o_cw04)=0.0
                    if (p_opcg4_b_o_cw04.gt.1)   chem(i,k,j,p_opcg4_b_o_cw04)=0.0
                    if (p_opcg5_b_o_cw04.gt.1)   chem(i,k,j,p_opcg5_b_o_cw04)=0.0
                    if (p_opcg6_b_o_cw04.gt.1)   chem(i,k,j,p_opcg6_b_o_cw04)=0.0

                    if (p_tanv_cw04.gt.1)        chem(i,k,j,p_tanv_cw04)=0.0
                    if (p_ternv_cw04.gt.1)       chem(i,k,j,p_ternv_cw04)=0.0
                    if (p_sopnv_cw04.gt.1)       chem(i,k,j,p_sopnv_cw04)=0.0
                    if (p_seqnv_cw04.gt.1)       chem(i,k,j,p_seqnv_cw04)=0.0
                    if (p_sianv_cw04.gt.1)       chem(i,k,j,p_sianv_cw04)=0.0
                    if (p_iepox_cw04.gt.1)       chem(i,k,j,p_iepox_cw04)=0.0
                    if (p_iepoxos_cw04.gt.1)    chem(i,k,j,p_iepoxos_cw04)=0.0
                    if (p_tetrol_cw04.gt.1)     chem(i,k,j,p_tetrol_cw04)=0.0
                    if (p_cldiepoxos_cw04.gt.1)    chem(i,k,j,p_cldiepoxos_cw04)=0.0
                    if (p_cldtetrol_cw04.gt.1)     chem(i,k,j,p_cldtetrol_cw04)=0.0
                    if (p_gly_cw04.gt.1)        chem(i,k,j,p_gly_cw04)=0.0
                    ...
                    if (p_ant2_c_cw05.gt.1)      chem(i,k,j,p_ant2_c_cw05)=0.0
                    if (p_ant3_c_cw05.gt.1)      chem(i,k,j,p_ant3_c_cw05)=0.0
                    if (p_ant4_c_cw05.gt.1)      chem(i,k,j,p_ant4_c_cw05)=0.0
                    if (p_biog1_c_cw05.gt.1)     chem(i,k,j,p_biog1_c_cw05)=0.0
                    if (p_biog2_c_cw05.gt.1)     chem(i,k,j,p_biog2_c_cw05)=0.0
                    if (p_biog3_c_cw05.gt.1)     chem(i,k,j,p_biog3_c_cw05)=0.0
                    if (p_biog4_c_cw05.gt.1)     chem(i,k,j,p_biog4_c_cw05)=0.0
                    if (p_biog1_o_cw05.gt.1)     chem(i,k,j,p_biog1_o_cw05)=0.0
                    if (p_biog2_o_cw05.gt.1)     chem(i,k,j,p_biog2_o_cw05)=0.0
                    if (p_biog3_o_cw05.gt.1)     chem(i,k,j,p_biog3_o_cw05)=0.0
                    if (p_biog4_o_cw05.gt.1)     chem(i,k,j,p_biog4_o_cw05)=0.0
                    if (p_tanv_cw05.gt.1)        chem(i,k,j,p_tanv_cw05)=0.0
                    if (p_ternv_cw05.gt.1)       chem(i,k,j,p_ternv_cw05)=0.0
                    if (p_sopnv_cw05.gt.1)      chem(i,k,j,p_sopnv_cw05)=0.0
                    if (p_seqnv_cw05.gt.1)     chem(i,k,j,p_seqnv_cw05)=0.0
                    if (p_sianv_cw05.gt.1)    chem(i,k,j,p_sianv_cw05)=0.0
                    if (p_iepox_cw05.gt.1)    chem(i,k,j,p_iepox_cw05)=0.0
                    if (p_iepoxos_cw05.gt.1)    chem(i,k,j,p_iepoxos_cw05)=0.0
                    if (p_tetrol_cw05.gt.1)    chem(i,k,j,p_tetrol_cw05)=0.0
                    if (p_gly_cw05.gt.1)    chem(i,k,j,p_gly_cw05)=0.0
                    ...
                    if (p_ant2_c_cw06.gt.1)      chem(i,k,j,p_ant2_c_cw06)=0.0
                    if (p_ant3_c_cw06.gt.1)      chem(i,k,j,p_ant3_c_cw06)=0.0
                    if (p_ant4_c_cw06.gt.1)      chem(i,k,j,p_ant4_c_cw06)=0.0
                    if (p_biog1_c_cw06.gt.1)     chem(i,k,j,p_biog1_c_cw06)=0.0
                    if (p_biog2_c_cw06.gt.1)     chem(i,k,j,p_biog2_c_cw06)=0.0
                    if (p_biog3_c_cw06.gt.1)     chem(i,k,j,p_biog3_c_cw06)=0.0
                    if (p_biog4_c_cw06.gt.1)     chem(i,k,j,p_biog4_c_cw06)=0.0
                    if (p_biog1_o_cw06.gt.1)     chem(i,k,j,p_biog1_o_cw06)=0.0
                    if (p_biog2_o_cw06.gt.1)     chem(i,k,j,p_biog2_o_cw06)=0.0
                    if (p_biog3_o_cw06.gt.1)     chem(i,k,j,p_biog3_o_cw06)=0.0
                    if (p_biog4_o_cw06.gt.1)     chem(i,k,j,p_biog4_o_cw06)=0.0
                    if (p_tanv_cw06.gt.1)        chem(i,k,j,p_tanv_cw06)=0.0
                    if (p_ternv_cw06.gt.1)       chem(i,k,j,p_ternv_cw06)=0.0
                    if (p_sopnv_cw06.gt.1)       chem(i,k,j,p_sopnv_cw06)=0.0
                    if (p_seqnv_cw06.gt.1)       chem(i,k,j,p_seqnv_cw06)=0.0
                    if (p_sianv_cw06.gt.1)       chem(i,k,j,p_sianv_cw06)=0.0
                    if (p_iepox_cw06.gt.1)       chem(i,k,j,p_iepox_cw06)=0.0
                    if (p_iepoxos_cw06.gt.1)    chem(i,k,j,p_iepoxos_cw06)=0.0
                    if (p_tetrol_cw06.gt.1)     chem(i,k,j,p_tetrol_cw06)=0.0
                    if (p_gly_cw06.gt.1)       chem(i,k,j,p_gly_cw06)=0.0
                    ...
                    if (p_ant2_c_cw07.gt.1)      chem(i,k,j,p_ant2_c_cw07)=0.0
                    if (p_ant3_c_cw07.gt.1)      chem(i,k,j,p_ant3_c_cw07)=0.0
                    if (p_ant4_c_cw07.gt.1)      chem(i,k,j,p_ant4_c_cw07)=0.0
                    if (p_biog1_c_cw07.gt.1)     chem(i,k,j,p_biog1_c_cw07)=0.0
                    if (p_biog2_c_cw07.gt.1)     chem(i,k,j,p_biog2_c_cw07)=0.0
                    if (p_biog3_c_cw07.gt.1)     chem(i,k,j,p_biog3_c_cw07)=0.0
                    if (p_biog4_c_cw07.gt.1)     chem(i,k,j,p_biog4_c_cw07)=0.0
                    if (p_biog1_o_cw07.gt.1)     chem(i,k,j,p_biog1_o_cw07)=0.0
                    if (p_biog2_o_cw07.gt.1)     chem(i,k,j,p_biog2_o_cw07)=0.0
                    if (p_biog3_o_cw07.gt.1)     chem(i,k,j,p_biog3_o_cw07)=0.0
                    if (p_biog4_o_cw07.gt.1)     chem(i,k,j,p_biog4_o_cw07)=0.0
                    if (p_tanv_cw07.gt.1)        chem(i,k,j,p_tanv_cw07)=0.0
                    if (p_ternv_cw07.gt.1)       chem(i,k,j,p_ternv_cw07)=0.0
                    if (p_sopnv_cw07.gt.1)       chem(i,k,j,p_sopnv_cw07)=0.0
                    if (p_seqnv_cw07.gt.1)       chem(i,k,j,p_seqnv_cw07)=0.0
                    if (p_sianv_cw07.gt.1)       chem(i,k,j,p_sianv_cw07)=0.0
                    if (p_iepox_cw07.gt.1)       chem(i,k,j,p_iepox_cw07)=0.0
                    if (p_iepoxos_cw07.gt.1)    chem(i,k,j,p_iepoxos_cw07)=0.0
                    if (p_tetrol_cw07.gt.1)    chem(i,k,j,p_tetrol_cw07)=0.0
                    if (p_gly_cw07.gt.1)       chem(i,k,j,p_gly_cw07)=0.0
                    ...
                    if (p_ant2_c_cw08.gt.1)      chem(i,k,j,p_ant2_c_cw08)=0.0
                    if (p_ant3_c_cw08.gt.1)      chem(i,k,j,p_ant3_c_cw08)=0.0
                    if (p_ant4_c_cw08.gt.1)      chem(i,k,j,p_ant4_c_cw08)=0.0
                    if (p_biog1_c_cw08.gt.1)     chem(i,k,j,p_biog1_c_cw08)=0.0
                    if (p_biog2_c_cw08.gt.1)     chem(i,k,j,p_biog2_c_cw08)=0.0
                    if (p_biog3_c_cw08.gt.1)     chem(i,k,j,p_biog3_c_cw08)=0.0
                    if (p_biog4_c_cw08.gt.1)     chem(i,k,j,p_biog4_c_cw08)=0.0
                    if (p_biog1_o_cw08.gt.1)     chem(i,k,j,p_biog1_o_cw08)=0.0
                    if (p_biog2_o_cw08.gt.1)     chem(i,k,j,p_biog2_o_cw08)=0.0
                    if (p_biog3_o_cw08.gt.1)     chem(i,k,j,p_biog3_o_cw08)=0.0
                    if (p_biog4_o_cw08.gt.1)     chem(i,k,j,p_biog4_o_cw08)=0.0
                    if (p_tanv_cw08.gt.1)        chem(i,k,j,p_tanv_cw08)=0.0
                    if (p_ternv_cw08.gt.1)      chem(i,k,j,p_ternv_cw08)=0.0
                    if (p_sopnv_cw08.gt.1)      chem(i,k,j,p_sopnv_cw08)=0.0
                    if (p_seqnv_cw08.gt.1)      chem(i,k,j,p_seqnv_cw08)=0.0
                    if (p_sianv_cw08.gt.1)      chem(i,k,j,p_sianv_cw08)=0.0
                    if (p_iepox_cw08.gt.1)      chem(i,k,j,p_iepox_cw08)=0.0
                    if (p_iepoxos_cw08.gt.1)    chem(i,k,j,p_iepoxos_cw08)=0.0
                    if (p_tetrol_cw08.gt.1)    chem(i,k,j,p_tetrol_cw08)=0.0
                    if (p_gly_cw08.gt.1)       chem(i,k,j,p_gly_cw08)=0.0

                    if (p_dms.gt.1)  chem(i,k,j,p_dms)=0.0
        ...
        CASE (SAPRC99_MOSAIC_4BIN_VBS2_KPP)
       if(config_flags%chem_in_opt == 1 ) grid%vbs_nbin(1)=2   
        if(config_flags%chem_in_opt == 0 )then
                   grid%vbs_nbin(1)=2
          do j=jts,jte
             do k=kts,kte
    ...
    CASE (SAPRC99_MOSAIC_8BIN_VBS2_KPP,  &
          SAPRC99_MOSAIC_20BIN_VBS2_KPP,SAPRC99_MOSAIC_12BIN_VBS2_KPP) 
       if(config_flags%chem_in_opt == 1 ) grid%vbs_nbin(1)=2   
       if(config_flags%chem_in_opt == 0 )then
          grid%vbs_nbin(1)=2
          do j=jts,jte
             do k=kts,kte
                do i=its,ite
                   ...
                   if (p_pcg1_b_c_a09.gt.1)    chem(i,k,j,p_pcg1_b_c_a09)=0.0
                   if (p_pcg1_b_o_a09.gt.1)    chem(i,k,j,p_pcg1_b_o_a09)=0.0
                   if (p_opcg1_b_c_a09.gt.1)    chem(i,k,j,p_opcg1_b_c_a09)=0.0
                   if (p_opcg1_b_o_a09.gt.1)    chem(i,k,j,p_opcg1_b_o_a09)=0.0
                   if (p_pcg1_f_c_a09.gt.1)    chem(i,k,j,p_pcg1_f_c_a09)=0.0
                   if (p_pcg1_f_o_a09.gt.1)    chem(i,k,j,p_pcg1_f_o_a09)=0.0
                   if (p_opcg1_f_c_a09.gt.1)    chem(i,k,j,p_opcg1_f_c_a09)=0.0
                   if (p_opcg1_f_o_a09.gt.1)    chem(i,k,j,p_opcg1_f_o_a09)=0.0
                   if (p_ant1_c_a09.gt.1)    chem(i,k,j,p_ant1_c_a09)=0.0
                   if (p_biog1_c_a09.gt.1)    chem(i,k,j,p_biog1_c_a09)=0.0
                   if (p_ant2_c_a09.gt.1)    chem(i,k,j,p_ant2_c_a09)=0.0
                   if (p_biog2_c_a09.gt.1)    chem(i,k,j,p_biog2_c_a09)=0.0
                   if (p_biog3_c_a09.gt.1)    chem(i,k,j,p_biog3_c_a09)=0.0
                   if (p_biog4_c_a09.gt.1)    chem(i,k,j,p_biog4_c_a09)=0.0
                   if (p_biog1_o_a09.gt.1)    chem(i,k,j,p_biog1_o_a09)=0.0
                   if (p_biog2_o_a09.gt.1)    chem(i,k,j,p_biog2_o_a09)=0.0
                   if (p_ant3_c_a09.gt.1)    chem(i,k,j,p_ant3_c_a09)=0.0
                   if (p_ant4_c_a09.gt.1)    chem(i,k,j,p_ant4_c_a09)=0.0
                   
                   if (p_pcg1_b_c_a10.gt.1)    chem(i,k,j,p_pcg1_b_c_a10)=0.0
                   if (p_pcg1_b_o_a10.gt.1)    chem(i,k,j,p_pcg1_b_o_a10)=0.0
                   if (p_opcg1_b_c_a10.gt.1)    chem(i,k,j,p_opcg1_b_c_a10)=0.0
                   if (p_opcg1_b_o_a10.gt.1)    chem(i,k,j,p_opcg1_b_o_a10)=0.0
                   if (p_pcg1_f_c_a10.gt.1)    chem(i,k,j,p_pcg1_f_c_a10)=0.0
                   if (p_pcg1_f_o_a10.gt.1)    chem(i,k,j,p_pcg1_f_o_a10)=0.0
                   if (p_opcg1_f_c_a10.gt.1)    chem(i,k,j,p_opcg1_f_c_a10)=0.0
                   if (p_opcg1_f_o_a10.gt.1)    chem(i,k,j,p_opcg1_f_o_a10)=0.0
                   if (p_ant1_c_a10.gt.1)    chem(i,k,j,p_ant1_c_a10)=0.0
                   if (p_biog1_c_a10.gt.1)    chem(i,k,j,p_biog1_c_a10)=0.0
                   if (p_ant2_c_a10.gt.1)    chem(i,k,j,p_ant2_c_a10)=0.0
                   if (p_biog2_c_a10.gt.1)    chem(i,k,j,p_biog2_c_a10)=0.0
                   if (p_biog3_c_a10.gt.1)    chem(i,k,j,p_biog3_c_a10)=0.0
                   if (p_biog4_c_a10.gt.1)    chem(i,k,j,p_biog4_c_a10)=0.0
                   if (p_biog1_o_a10.gt.1)    chem(i,k,j,p_biog1_o_a10)=0.0
                   if (p_biog2_o_a10.gt.1)    chem(i,k,j,p_biog2_o_a10)=0.0
                   if (p_ant3_c_a10.gt.1)    chem(i,k,j,p_ant3_c_a10)=0.0
                   if (p_ant4_c_a10.gt.1)    chem(i,k,j,p_ant4_c_a10)=0.0

                   if (p_pcg1_b_c_a11.gt.1)    chem(i,k,j,p_pcg1_b_c_a11)=0.0
                   if (p_pcg1_b_o_a11.gt.1)    chem(i,k,j,p_pcg1_b_o_a11)=0.0
                   if (p_opcg1_b_c_a11.gt.1)    chem(i,k,j,p_opcg1_b_c_a11)=0.0
                   if (p_opcg1_b_o_a11.gt.1)    chem(i,k,j,p_opcg1_b_o_a11)=0.0
                   if (p_pcg1_f_c_a11.gt.1)    chem(i,k,j,p_pcg1_f_c_a11)=0.0
                   if (p_pcg1_f_o_a11.gt.1)    chem(i,k,j,p_pcg1_f_o_a11)=0.0
                   if (p_opcg1_f_c_a11.gt.1)    chem(i,k,j,p_opcg1_f_c_a11)=0.0
                   if (p_opcg1_f_o_a11.gt.1)    chem(i,k,j,p_opcg1_f_o_a11)=0.0
                   if (p_ant1_c_a11.gt.1)    chem(i,k,j,p_ant1_c_a11)=0.0
                   if (p_biog1_c_a11.gt.1)    chem(i,k,j,p_biog1_c_a11)=0.0
                   if (p_ant2_c_a11.gt.1)    chem(i,k,j,p_ant2_c_a11)=0.0
                   if (p_biog2_c_a11.gt.1)    chem(i,k,j,p_biog2_c_a11)=0.0
                   if (p_biog3_c_a11.gt.1)    chem(i,k,j,p_biog3_c_a11)=0.0
                   if (p_biog4_c_a11.gt.1)    chem(i,k,j,p_biog4_c_a11)=0.0
                   if (p_biog1_o_a11.gt.1)    chem(i,k,j,p_biog1_o_a11)=0.0
                   if (p_biog2_o_a11.gt.1)    chem(i,k,j,p_biog2_o_a11)=0.0
                   if (p_ant3_c_a11.gt.1)    chem(i,k,j,p_ant3_c_a11)=0.0
                   if (p_ant4_c_a11.gt.1)    chem(i,k,j,p_ant4_c_a11)=0.0

                   if (p_pcg1_b_c_a12.gt.1)    chem(i,k,j,p_pcg1_b_c_a12)=0.0
                   if (p_pcg1_b_o_a12.gt.1)    chem(i,k,j,p_pcg1_b_o_a12)=0.0
                   if (p_opcg1_b_c_a12.gt.1)    chem(i,k,j,p_opcg1_b_c_a12)=0.0
                   if (p_opcg1_b_o_a12.gt.1)    chem(i,k,j,p_opcg1_b_o_a12)=0.0
                   if (p_pcg1_f_c_a12.gt.1)    chem(i,k,j,p_pcg1_f_c_a12)=0.0
                   if (p_pcg1_f_o_a12.gt.1)    chem(i,k,j,p_pcg1_f_o_a12)=0.0
                   if (p_opcg1_f_c_a12.gt.1)    chem(i,k,j,p_opcg1_f_c_a12)=0.0
                   if (p_opcg1_f_o_a12.gt.1)    chem(i,k,j,p_opcg1_f_o_a12)=0.0
                   if (p_ant1_c_a12.gt.1)    chem(i,k,j,p_ant1_c_a12)=0.0
                   if (p_biog1_c_a12.gt.1)    chem(i,k,j,p_biog1_c_a12)=0.0
                   if (p_ant2_c_a12.gt.1)    chem(i,k,j,p_ant2_c_a12)=0.0
                   if (p_biog2_c_a12.gt.1)    chem(i,k,j,p_biog2_c_a12)=0.0
                   if (p_biog3_c_a12.gt.1)    chem(i,k,j,p_biog3_c_a12)=0.0
                   if (p_biog4_c_a12.gt.1)    chem(i,k,j,p_biog4_c_a12)=0.0
                   if (p_biog1_o_a12.gt.1)    chem(i,k,j,p_biog1_o_a12)=0.0
                   if (p_biog2_o_a12.gt.1)    chem(i,k,j,p_biog2_o_a12)=0.0
                   if (p_ant3_c_a12.gt.1)    chem(i,k,j,p_ant3_c_a12)=0.0
                   if (p_ant4_c_a12.gt.1)    chem(i,k,j,p_ant4_c_a12)=0.0

                   if (p_pcg1_b_c_a13.gt.1)    chem(i,k,j,p_pcg1_b_c_a13)=0.0
                   if (p_pcg1_b_o_a13.gt.1)    chem(i,k,j,p_pcg1_b_o_a13)=0.0
                   if (p_opcg1_b_c_a13.gt.1)    chem(i,k,j,p_opcg1_b_c_a13)=0.0
                   if (p_opcg1_b_o_a13.gt.1)    chem(i,k,j,p_opcg1_b_o_a13)=0.0
                   if (p_pcg1_f_c_a13.gt.1)    chem(i,k,j,p_pcg1_f_c_a13)=0.0
                   if (p_pcg1_f_o_a13.gt.1)    chem(i,k,j,p_pcg1_f_o_a13)=0.0
                   if (p_opcg1_f_c_a13.gt.1)    chem(i,k,j,p_opcg1_f_c_a13)=0.0
                   if (p_opcg1_f_o_a13.gt.1)    chem(i,k,j,p_opcg1_f_o_a13)=0.0
                   if (p_ant1_c_a13.gt.1)    chem(i,k,j,p_ant1_c_a13)=0.0
                   if (p_biog1_c_a13.gt.1)    chem(i,k,j,p_biog1_c_a13)=0.0
                   if (p_ant2_c_a13.gt.1)    chem(i,k,j,p_ant2_c_a13)=0.0
                   if (p_biog2_c_a13.gt.1)    chem(i,k,j,p_biog2_c_a13)=0.0
                   if (p_biog3_c_a13.gt.1)    chem(i,k,j,p_biog3_c_a13)=0.0
                   if (p_biog4_c_a13.gt.1)    chem(i,k,j,p_biog4_c_a13)=0.0
                   if (p_biog1_o_a13.gt.1)    chem(i,k,j,p_biog1_o_a13)=0.0
                   if (p_biog2_o_a13.gt.1)    chem(i,k,j,p_biog2_o_a13)=0.0
                   if (p_ant3_c_a13.gt.1)    chem(i,k,j,p_ant3_c_a13)=0.0
                   if (p_ant4_c_a13.gt.1)    chem(i,k,j,p_ant4_c_a13)=0.0

                   if (p_pcg1_b_c_a14.gt.1)    chem(i,k,j,p_pcg1_b_c_a14)=0.0
                   if (p_pcg1_b_o_a14.gt.1)    chem(i,k,j,p_pcg1_b_o_a14)=0.0
                   if (p_opcg1_b_c_a14.gt.1)    chem(i,k,j,p_opcg1_b_c_a14)=0.0
                   if (p_opcg1_b_o_a14.gt.1)    chem(i,k,j,p_opcg1_b_o_a14)=0.0
                   if (p_pcg1_f_c_a14.gt.1)    chem(i,k,j,p_pcg1_f_c_a14)=0.0
                   if (p_pcg1_f_o_a14.gt.1)    chem(i,k,j,p_pcg1_f_o_a14)=0.0
                   if (p_opcg1_f_c_a14.gt.1)    chem(i,k,j,p_opcg1_f_c_a14)=0.0
                   if (p_opcg1_f_o_a14.gt.1)    chem(i,k,j,p_opcg1_f_o_a14)=0.0
                   if (p_ant1_c_a14.gt.1)    chem(i,k,j,p_ant1_c_a14)=0.0
                   if (p_biog1_c_a14.gt.1)    chem(i,k,j,p_biog1_c_a14)=0.0
                   if (p_ant2_c_a14.gt.1)    chem(i,k,j,p_ant2_c_a14)=0.0
                   if (p_biog2_c_a14.gt.1)    chem(i,k,j,p_biog2_c_a14)=0.0
                   if (p_biog3_c_a14.gt.1)    chem(i,k,j,p_biog3_c_a14)=0.0
                   if (p_biog4_c_a14.gt.1)    chem(i,k,j,p_biog4_c_a14)=0.0
                   if (p_biog1_o_a14.gt.1)    chem(i,k,j,p_biog1_o_a14)=0.0
                   if (p_biog2_o_a14.gt.1)    chem(i,k,j,p_biog2_o_a14)=0.0
                   if (p_ant3_c_a14.gt.1)    chem(i,k,j,p_ant3_c_a14)=0.0
                   if (p_ant4_c_a14.gt.1)    chem(i,k,j,p_ant4_c_a14)=0.0

                   if (p_pcg1_b_c_a15.gt.1)    chem(i,k,j,p_pcg1_b_c_a15)=0.0
                   if (p_pcg1_b_o_a15.gt.1)    chem(i,k,j,p_pcg1_b_o_a15)=0.0
                   if (p_opcg1_b_c_a15.gt.1)    chem(i,k,j,p_opcg1_b_c_a15)=0.0
                   if (p_opcg1_b_o_a15.gt.1)    chem(i,k,j,p_opcg1_b_o_a15)=0.0
                   if (p_pcg1_f_c_a15.gt.1)    chem(i,k,j,p_pcg1_f_c_a15)=0.0
                   if (p_pcg1_f_o_a15.gt.1)    chem(i,k,j,p_pcg1_f_o_a15)=0.0
                   if (p_opcg1_f_c_a15.gt.1)    chem(i,k,j,p_opcg1_f_c_a15)=0.0
                   if (p_opcg1_f_o_a15.gt.1)    chem(i,k,j,p_opcg1_f_o_a15)=0.0
                   if (p_ant1_c_a15.gt.1)    chem(i,k,j,p_ant1_c_a15)=0.0
                   if (p_biog1_c_a15.gt.1)    chem(i,k,j,p_biog1_c_a15)=0.0
                   if (p_ant2_c_a15.gt.1)    chem(i,k,j,p_ant2_c_a15)=0.0
                   if (p_biog2_c_a15.gt.1)    chem(i,k,j,p_biog2_c_a15)=0.0
                   if (p_biog3_c_a15.gt.1)    chem(i,k,j,p_biog3_c_a15)=0.0
                   if (p_biog4_c_a15.gt.1)    chem(i,k,j,p_biog4_c_a15)=0.0
                   if (p_biog1_o_a15.gt.1)    chem(i,k,j,p_biog1_o_a15)=0.0
                   if (p_biog2_o_a15.gt.1)    chem(i,k,j,p_biog2_o_a15)=0.0
                   if (p_ant3_c_a15.gt.1)    chem(i,k,j,p_ant3_c_a15)=0.0
                   if (p_ant4_c_a15.gt.1)    chem(i,k,j,p_ant4_c_a15)=0.0

                   if (p_pcg1_b_c_a16.gt.1)    chem(i,k,j,p_pcg1_b_c_a16)=0.0
                   if (p_pcg1_b_o_a16.gt.1)    chem(i,k,j,p_pcg1_b_o_a16)=0.0
                   if (p_opcg1_b_c_a16.gt.1)    chem(i,k,j,p_opcg1_b_c_a16)=0.0
                   if (p_opcg1_b_o_a16.gt.1)    chem(i,k,j,p_opcg1_b_o_a16)=0.0
                   if (p_pcg1_f_c_a16.gt.1)    chem(i,k,j,p_pcg1_f_c_a16)=0.0
                   if (p_pcg1_f_o_a16.gt.1)    chem(i,k,j,p_pcg1_f_o_a16)=0.0
                   if (p_opcg1_f_c_a16.gt.1)    chem(i,k,j,p_opcg1_f_c_a16)=0.0
                   if (p_opcg1_f_o_a16.gt.1)    chem(i,k,j,p_opcg1_f_o_a16)=0.0
                   if (p_ant1_c_a16.gt.1)    chem(i,k,j,p_ant1_c_a16)=0.0
                   if (p_biog1_c_a16.gt.1)    chem(i,k,j,p_biog1_c_a16)=0.0
                   if (p_ant2_c_a16.gt.1)    chem(i,k,j,p_ant2_c_a16)=0.0
                   if (p_biog2_c_a16.gt.1)    chem(i,k,j,p_biog2_c_a16)=0.0
                   if (p_biog3_c_a16.gt.1)    chem(i,k,j,p_biog3_c_a16)=0.0
                   if (p_biog4_c_a16.gt.1)    chem(i,k,j,p_biog4_c_a16)=0.0
                   if (p_biog1_o_a16.gt.1)    chem(i,k,j,p_biog1_o_a16)=0.0
                   if (p_biog2_o_a16.gt.1)    chem(i,k,j,p_biog2_o_a16)=0.0
                   if (p_ant3_c_a16.gt.1)    chem(i,k,j,p_ant3_c_a16)=0.0
                   if (p_ant4_c_a16.gt.1)    chem(i,k,j,p_ant4_c_a16)=0.0

                   if (p_pcg1_b_c_a17.gt.1)    chem(i,k,j,p_pcg1_b_c_a17)=0.0
                   if (p_pcg1_b_o_a17.gt.1)    chem(i,k,j,p_pcg1_b_o_a17)=0.0
                   if (p_opcg1_b_c_a17.gt.1)    chem(i,k,j,p_opcg1_b_c_a17)=0.0
                   if (p_opcg1_b_o_a17.gt.1)    chem(i,k,j,p_opcg1_b_o_a17)=0.0
                   if (p_pcg1_f_c_a17.gt.1)    chem(i,k,j,p_pcg1_f_c_a17)=0.0
                   if (p_pcg1_f_o_a17.gt.1)    chem(i,k,j,p_pcg1_f_o_a17)=0.0
                   if (p_opcg1_f_c_a17.gt.1)    chem(i,k,j,p_opcg1_f_c_a17)=0.0
                   if (p_opcg1_f_o_a17.gt.1)    chem(i,k,j,p_opcg1_f_o_a17)=0.0
                   if (p_ant1_c_a17.gt.1)    chem(i,k,j,p_ant1_c_a17)=0.0
                   if (p_biog1_c_a17.gt.1)    chem(i,k,j,p_biog1_c_a17)=0.0
                   if (p_ant2_c_a17.gt.1)    chem(i,k,j,p_ant2_c_a17)=0.0
                   if (p_biog2_c_a17.gt.1)    chem(i,k,j,p_biog2_c_a17)=0.0
                   if (p_biog3_c_a17.gt.1)    chem(i,k,j,p_biog3_c_a17)=0.0
                   if (p_biog4_c_a17.gt.1)    chem(i,k,j,p_biog4_c_a17)=0.0
                   if (p_biog1_o_a17.gt.1)    chem(i,k,j,p_biog1_o_a17)=0.0
                   if (p_biog2_o_a17.gt.1)    chem(i,k,j,p_biog2_o_a17)=0.0
                   if (p_ant3_c_a17.gt.1)    chem(i,k,j,p_ant3_c_a17)=0.0
                   if (p_ant4_c_a17.gt.1)    chem(i,k,j,p_ant4_c_a17)=0.0

                   if (p_pcg1_b_c_a18.gt.1)    chem(i,k,j,p_pcg1_b_c_a18)=0.0
                   if (p_pcg1_b_o_a18.gt.1)    chem(i,k,j,p_pcg1_b_o_a18)=0.0
                   if (p_opcg1_b_c_a18.gt.1)    chem(i,k,j,p_opcg1_b_c_a18)=0.0
                   if (p_opcg1_b_o_a18.gt.1)    chem(i,k,j,p_opcg1_b_o_a18)=0.0
                   if (p_pcg1_f_c_a18.gt.1)    chem(i,k,j,p_pcg1_f_c_a18)=0.0
                   if (p_pcg1_f_o_a18.gt.1)    chem(i,k,j,p_pcg1_f_o_a18)=0.0
                   if (p_opcg1_f_c_a18.gt.1)    chem(i,k,j,p_opcg1_f_c_a18)=0.0
                   if (p_opcg1_f_o_a18.gt.1)    chem(i,k,j,p_opcg1_f_o_a18)=0.0
                   if (p_ant1_c_a18.gt.1)    chem(i,k,j,p_ant1_c_a18)=0.0
                   if (p_biog1_c_a18.gt.1)    chem(i,k,j,p_biog1_c_a18)=0.0
                   if (p_ant2_c_a18.gt.1)    chem(i,k,j,p_ant2_c_a18)=0.0
                   if (p_biog2_c_a18.gt.1)    chem(i,k,j,p_biog2_c_a18)=0.0
                   if (p_biog3_c_a18.gt.1)    chem(i,k,j,p_biog3_c_a18)=0.0
                   if (p_biog4_c_a18.gt.1)    chem(i,k,j,p_biog4_c_a18)=0.0
                   if (p_biog1_o_a18.gt.1)    chem(i,k,j,p_biog1_o_a18)=0.0
                   if (p_biog2_o_a18.gt.1)    chem(i,k,j,p_biog2_o_a18)=0.0
                   if (p_ant3_c_a18.gt.1)    chem(i,k,j,p_ant3_c_a18)=0.0
                   if (p_ant4_c_a18.gt.1)    chem(i,k,j,p_ant4_c_a18)=0.0

                   if (p_pcg1_b_c_a19.gt.1)    chem(i,k,j,p_pcg1_b_c_a19)=0.0
                   if (p_pcg1_b_o_a19.gt.1)    chem(i,k,j,p_pcg1_b_o_a19)=0.0
                   if (p_opcg1_b_c_a19.gt.1)    chem(i,k,j,p_opcg1_b_c_a19)=0.0
                   if (p_opcg1_b_o_a19.gt.1)    chem(i,k,j,p_opcg1_b_o_a19)=0.0
                   if (p_pcg1_f_c_a19.gt.1)    chem(i,k,j,p_pcg1_f_c_a19)=0.0
                   if (p_pcg1_f_o_a19.gt.1)    chem(i,k,j,p_pcg1_f_o_a19)=0.0
                   if (p_opcg1_f_c_a19.gt.1)    chem(i,k,j,p_opcg1_f_c_a19)=0.0
                   if (p_opcg1_f_o_a19.gt.1)    chem(i,k,j,p_opcg1_f_o_a19)=0.0
                   if (p_ant1_c_a19.gt.1)    chem(i,k,j,p_ant1_c_a19)=0.0
                   if (p_biog1_c_a19.gt.1)    chem(i,k,j,p_biog1_c_a19)=0.0
                   if (p_ant2_c_a19.gt.1)    chem(i,k,j,p_ant2_c_a19)=0.0
                   if (p_biog2_c_a19.gt.1)    chem(i,k,j,p_biog2_c_a19)=0.0
                   if (p_biog3_c_a19.gt.1)    chem(i,k,j,p_biog3_c_a19)=0.0
                   if (p_biog4_c_a19.gt.1)    chem(i,k,j,p_biog4_c_a19)=0.0
                   if (p_biog1_o_a19.gt.1)    chem(i,k,j,p_biog1_o_a19)=0.0
                   if (p_biog2_o_a19.gt.1)    chem(i,k,j,p_biog2_o_a19)=0.0
                   if (p_ant3_c_a19.gt.1)    chem(i,k,j,p_ant3_c_a19)=0.0
                   if (p_ant4_c_a19.gt.1)    chem(i,k,j,p_ant4_c_a19)=0.0

                   if (p_pcg1_b_c_a20.gt.1)    chem(i,k,j,p_pcg1_b_c_a20)=0.0
                   if (p_pcg1_b_o_a20.gt.1)    chem(i,k,j,p_pcg1_b_o_a20)=0.0
                   if (p_opcg1_b_c_a20.gt.1)    chem(i,k,j,p_opcg1_b_c_a20)=0.0
                   if (p_opcg1_b_o_a20.gt.1)    chem(i,k,j,p_opcg1_b_o_a20)=0.0
                   if (p_pcg1_f_c_a20.gt.1)    chem(i,k,j,p_pcg1_f_c_a20)=0.0
                   if (p_pcg1_f_o_a20.gt.1)    chem(i,k,j,p_pcg1_f_o_a20)=0.0
                   if (p_opcg1_f_c_a20.gt.1)    chem(i,k,j,p_opcg1_f_c_a20)=0.0
                   if (p_opcg1_f_o_a20.gt.1)    chem(i,k,j,p_opcg1_f_o_a20)=0.0
                   if (p_ant1_c_a20.gt.1)    chem(i,k,j,p_ant1_c_a20)=0.0
                   if (p_biog1_c_a20.gt.1)    chem(i,k,j,p_biog1_c_a20)=0.0
                   if (p_ant2_c_a20.gt.1)    chem(i,k,j,p_ant2_c_a20)=0.0
                   if (p_biog2_c_a20.gt.1)    chem(i,k,j,p_biog2_c_a20)=0.0
                   if (p_biog3_c_a20.gt.1)    chem(i,k,j,p_biog3_c_a20)=0.0
                   if (p_biog4_c_a20.gt.1)    chem(i,k,j,p_biog4_c_a20)=0.0
                   if (p_biog1_o_a20.gt.1)    chem(i,k,j,p_biog1_o_a20)=0.0
                   if (p_biog2_o_a20.gt.1)    chem(i,k,j,p_biog2_o_a20)=0.0
                   if (p_ant3_c_a20.gt.1)    chem(i,k,j,p_ant3_c_a20)=0.0
                   if (p_ant4_c_a20.gt.1)    chem(i,k,j,p_ant4_c_a20)=0.0
          ...
          do j=jts,jte
             do k=kts,kte
                do i=its,ite
                   ...
                   if (p_pcg1_b_c_cw09.gt.1)   chem(i,k,j,p_pcg1_b_c_cw09)=0.0
                   if (p_pcg1_b_o_cw09.gt.1)   chem(i,k,j,p_pcg1_b_o_cw09)=0.0
                   if (p_opcg1_b_c_cw09.gt.1)  chem(i,k,j,p_opcg1_b_c_cw09)=0.0
                   if (p_opcg1_b_o_cw09.gt.1)  chem(i,k,j,p_opcg1_b_o_cw09)=0.0
                   if (p_pcg1_f_c_cw09.gt.1)   chem(i,k,j,p_pcg1_f_c_cw09)=0.0
                   if (p_pcg1_f_o_cw09.gt.1)   chem(i,k,j,p_pcg1_f_o_cw09)=0.0
                   if (p_opcg1_f_c_cw09.gt.1)  chem(i,k,j,p_opcg1_f_c_cw09)=0.0
                   if (p_opcg1_f_o_cw09.gt.1)  chem(i,k,j,p_opcg1_f_o_cw09)=0.0
                   if (p_ant1_c_cw09.gt.1)     chem(i,k,j,p_ant1_c_cw09)=0.0
                   if (p_biog1_c_cw09.gt.1)    chem(i,k,j,p_biog1_c_cw09)=0.0

                   if (p_pcg1_b_c_cw10.gt.1)   chem(i,k,j,p_pcg1_b_c_cw10)=0.0
                   if (p_pcg1_b_o_cw10.gt.1)   chem(i,k,j,p_pcg1_b_o_cw10)=0.0
                   if (p_opcg1_b_c_cw10.gt.1)  chem(i,k,j,p_opcg1_b_c_cw10)=0.0
                   if (p_opcg1_b_o_cw10.gt.1)  chem(i,k,j,p_opcg1_b_o_cw10)=0.0
                   if (p_pcg1_f_c_cw10.gt.1)   chem(i,k,j,p_pcg1_f_c_cw10)=0.0
                   if (p_pcg1_f_o_cw10.gt.1)   chem(i,k,j,p_pcg1_f_o_cw10)=0.0
                   if (p_opcg1_f_c_cw10.gt.1)  chem(i,k,j,p_opcg1_f_c_cw10)=0.0
                   if (p_opcg1_f_o_cw10.gt.1)  chem(i,k,j,p_opcg1_f_o_cw10)=0.0
                   if (p_ant1_c_cw10.gt.1)     chem(i,k,j,p_ant1_c_cw10)=0.0
                   if (p_biog1_c_cw10.gt.1)    chem(i,k,j,p_biog1_c_cw10)=0.0

                   if (p_pcg1_b_c_cw11.gt.1)   chem(i,k,j,p_pcg1_b_c_cw11)=0.0
                   if (p_pcg1_b_o_cw11.gt.1)   chem(i,k,j,p_pcg1_b_o_cw11)=0.0
                   if (p_opcg1_b_c_cw11.gt.1)  chem(i,k,j,p_opcg1_b_c_cw11)=0.0
                   if (p_opcg1_b_o_cw11.gt.1)  chem(i,k,j,p_opcg1_b_o_cw11)=0.0
                   if (p_pcg1_f_c_cw11.gt.1)   chem(i,k,j,p_pcg1_f_c_cw11)=0.0
                   if (p_pcg1_f_o_cw11.gt.1)   chem(i,k,j,p_pcg1_f_o_cw11)=0.0
                   if (p_opcg1_f_c_cw11.gt.1)  chem(i,k,j,p_opcg1_f_c_cw11)=0.0
                   if (p_opcg1_f_o_cw11.gt.1)  chem(i,k,j,p_opcg1_f_o_cw11)=0.0
                   if (p_ant1_c_cw11.gt.1)     chem(i,k,j,p_ant1_c_cw11)=0.0
                   if (p_biog1_c_cw11.gt.1)    chem(i,k,j,p_biog1_c_cw11)=0.0

                   if (p_pcg1_b_c_cw12.gt.1)   chem(i,k,j,p_pcg1_b_c_cw12)=0.0
                   if (p_pcg1_b_o_cw12.gt.1)   chem(i,k,j,p_pcg1_b_o_cw12)=0.0
                   if (p_opcg1_b_c_cw12.gt.1)  chem(i,k,j,p_opcg1_b_c_cw12)=0.0
                   if (p_opcg1_b_o_cw12.gt.1)  chem(i,k,j,p_opcg1_b_o_cw12)=0.0
                   if (p_pcg1_f_c_cw12.gt.1)   chem(i,k,j,p_pcg1_f_c_cw12)=0.0
                   if (p_pcg1_f_o_cw12.gt.1)   chem(i,k,j,p_pcg1_f_o_cw12)=0.0
                   if (p_opcg1_f_c_cw12.gt.1)  chem(i,k,j,p_opcg1_f_c_cw12)=0.0
                   if (p_opcg1_f_o_cw12.gt.1)  chem(i,k,j,p_opcg1_f_o_cw12)=0.0
                   if (p_ant1_c_cw12.gt.1)     chem(i,k,j,p_ant1_c_cw12)=0.0
                   if (p_biog1_c_cw12.gt.1)    chem(i,k,j,p_biog1_c_cw12)=0.0

                   if (p_pcg1_b_c_cw13.gt.1)   chem(i,k,j,p_pcg1_b_c_cw13)=0.0
                   if (p_pcg1_b_o_cw13.gt.1)   chem(i,k,j,p_pcg1_b_o_cw13)=0.0
                   if (p_opcg1_b_c_cw13.gt.1)  chem(i,k,j,p_opcg1_b_c_cw13)=0.0
                   if (p_opcg1_b_o_cw13.gt.1)  chem(i,k,j,p_opcg1_b_o_cw13)=0.0
                   if (p_pcg1_f_c_cw13.gt.1)   chem(i,k,j,p_pcg1_f_c_cw13)=0.0
                   if (p_pcg1_f_o_cw13.gt.1)   chem(i,k,j,p_pcg1_f_o_cw13)=0.0
                   if (p_opcg1_f_c_cw13.gt.1)  chem(i,k,j,p_opcg1_f_c_cw13)=0.0
                   if (p_opcg1_f_o_cw13.gt.1)  chem(i,k,j,p_opcg1_f_o_cw13)=0.0
                   if (p_ant1_c_cw13.gt.1)     chem(i,k,j,p_ant1_c_cw13)=0.0
                   if (p_biog1_c_cw13.gt.1)    chem(i,k,j,p_biog1_c_cw13)=0.0

                   if (p_pcg1_b_c_cw14.gt.1)   chem(i,k,j,p_pcg1_b_c_cw14)=0.0
                   if (p_pcg1_b_o_cw14.gt.1)   chem(i,k,j,p_pcg1_b_o_cw14)=0.0
                   if (p_opcg1_b_c_cw14.gt.1)  chem(i,k,j,p_opcg1_b_c_cw14)=0.0
                   if (p_opcg1_b_o_cw14.gt.1)  chem(i,k,j,p_opcg1_b_o_cw14)=0.0
                   if (p_pcg1_f_c_cw14.gt.1)   chem(i,k,j,p_pcg1_f_c_cw14)=0.0
                   if (p_pcg1_f_o_cw14.gt.1)   chem(i,k,j,p_pcg1_f_o_cw14)=0.0
                   if (p_opcg1_f_c_cw14.gt.1)  chem(i,k,j,p_opcg1_f_c_cw14)=0.0
                   if (p_opcg1_f_o_cw14.gt.1)  chem(i,k,j,p_opcg1_f_o_cw14)=0.0
                   if (p_ant1_c_cw14.gt.1)     chem(i,k,j,p_ant1_c_cw14)=0.0
                   if (p_biog1_c_cw14.gt.1)    chem(i,k,j,p_biog1_c_cw14)=0.0

                   if (p_pcg1_b_c_cw15.gt.1)   chem(i,k,j,p_pcg1_b_c_cw15)=0.0
                   if (p_pcg1_b_o_cw15.gt.1)   chem(i,k,j,p_pcg1_b_o_cw15)=0.0
                   if (p_opcg1_b_c_cw15.gt.1)  chem(i,k,j,p_opcg1_b_c_cw15)=0.0
                   if (p_opcg1_b_o_cw15.gt.1)  chem(i,k,j,p_opcg1_b_o_cw15)=0.0
                   if (p_pcg1_f_c_cw15.gt.1)   chem(i,k,j,p_pcg1_f_c_cw15)=0.0
                   if (p_pcg1_f_o_cw15.gt.1)   chem(i,k,j,p_pcg1_f_o_cw15)=0.0
                   if (p_opcg1_f_c_cw15.gt.1)  chem(i,k,j,p_opcg1_f_c_cw15)=0.0
                   if (p_opcg1_f_o_cw15.gt.1)  chem(i,k,j,p_opcg1_f_o_cw15)=0.0
                   if (p_ant1_c_cw15.gt.1)     chem(i,k,j,p_ant1_c_cw15)=0.0
                   if (p_biog1_c_cw15.gt.1)    chem(i,k,j,p_biog1_c_cw15)=0.0

                   if (p_pcg1_b_c_cw16.gt.1)   chem(i,k,j,p_pcg1_b_c_cw16)=0.0
                   if (p_pcg1_b_o_cw16.gt.1)   chem(i,k,j,p_pcg1_b_o_cw16)=0.0
                   if (p_opcg1_b_c_cw16.gt.1)  chem(i,k,j,p_opcg1_b_c_cw16)=0.0
                   if (p_opcg1_b_o_cw16.gt.1)  chem(i,k,j,p_opcg1_b_o_cw16)=0.0
                   if (p_pcg1_f_c_cw16.gt.1)   chem(i,k,j,p_pcg1_f_c_cw16)=0.0
                   if (p_pcg1_f_o_cw16.gt.1)   chem(i,k,j,p_pcg1_f_o_cw16)=0.0
                   if (p_opcg1_f_c_cw16.gt.1)  chem(i,k,j,p_opcg1_f_c_cw16)=0.0
                   if (p_opcg1_f_o_cw16.gt.1)  chem(i,k,j,p_opcg1_f_o_cw16)=0.0
                   if (p_ant1_c_cw16.gt.1)     chem(i,k,j,p_ant1_c_cw16)=0.0
                   if (p_biog1_c_cw16.gt.1)    chem(i,k,j,p_biog1_c_cw16)=0.0

                   if (p_pcg1_b_c_cw17.gt.1)   chem(i,k,j,p_pcg1_b_c_cw17)=0.0
                   if (p_pcg1_b_o_cw17.gt.1)   chem(i,k,j,p_pcg1_b_o_cw17)=0.0
                   if (p_opcg1_b_c_cw17.gt.1)  chem(i,k,j,p_opcg1_b_c_cw17)=0.0
                   if (p_opcg1_b_o_cw17.gt.1)  chem(i,k,j,p_opcg1_b_o_cw17)=0.0
                   if (p_pcg1_f_c_cw17.gt.1)   chem(i,k,j,p_pcg1_f_c_cw17)=0.0
                   if (p_pcg1_f_o_cw17.gt.1)   chem(i,k,j,p_pcg1_f_o_cw17)=0.0
                   if (p_opcg1_f_c_cw17.gt.1)  chem(i,k,j,p_opcg1_f_c_cw17)=0.0
                   if (p_opcg1_f_o_cw17.gt.1)  chem(i,k,j,p_opcg1_f_o_cw17)=0.0
                   if (p_ant1_c_cw17.gt.1)     chem(i,k,j,p_ant1_c_cw17)=0.0
                   if (p_biog1_c_cw17.gt.1)    chem(i,k,j,p_biog1_c_cw17)=0.0

                   if (p_pcg1_b_c_cw18.gt.1)   chem(i,k,j,p_pcg1_b_c_cw18)=0.0
                   if (p_pcg1_b_o_cw18.gt.1)   chem(i,k,j,p_pcg1_b_o_cw18)=0.0
                   if (p_opcg1_b_c_cw18.gt.1)  chem(i,k,j,p_opcg1_b_c_cw18)=0.0
                   if (p_opcg1_b_o_cw18.gt.1)  chem(i,k,j,p_opcg1_b_o_cw18)=0.0
                   if (p_pcg1_f_c_cw18.gt.1)   chem(i,k,j,p_pcg1_f_c_cw18)=0.0
                   if (p_pcg1_f_o_cw18.gt.1)   chem(i,k,j,p_pcg1_f_o_cw18)=0.0
                   if (p_opcg1_f_c_cw18.gt.1)  chem(i,k,j,p_opcg1_f_c_cw18)=0.0
                   if (p_opcg1_f_o_cw18.gt.1)  chem(i,k,j,p_opcg1_f_o_cw18)=0.0
                   if (p_ant1_c_cw18.gt.1)     chem(i,k,j,p_ant1_c_cw18)=0.0
                   if (p_biog1_c_cw18.gt.1)    chem(i,k,j,p_biog1_c_cw18)=0.0

                   if (p_pcg1_b_c_cw19.gt.1)   chem(i,k,j,p_pcg1_b_c_cw19)=0.0
                   if (p_pcg1_b_o_cw19.gt.1)   chem(i,k,j,p_pcg1_b_o_cw19)=0.0
                   if (p_opcg1_b_c_cw19.gt.1)  chem(i,k,j,p_opcg1_b_c_cw19)=0.0
                   if (p_opcg1_b_o_cw19.gt.1)  chem(i,k,j,p_opcg1_b_o_cw19)=0.0
                   if (p_pcg1_f_c_cw19.gt.1)   chem(i,k,j,p_pcg1_f_c_cw19)=0.0
                   if (p_pcg1_f_o_cw19.gt.1)   chem(i,k,j,p_pcg1_f_o_cw19)=0.0
                   if (p_opcg1_f_c_cw19.gt.1)  chem(i,k,j,p_opcg1_f_c_cw19)=0.0
                   if (p_opcg1_f_o_cw19.gt.1)  chem(i,k,j,p_opcg1_f_o_cw19)=0.0
                   if (p_ant1_c_cw19.gt.1)     chem(i,k,j,p_ant1_c_cw19)=0.0
                   if (p_biog1_c_cw19.gt.1)    chem(i,k,j,p_biog1_c_cw19)=0.0

                   if (p_pcg1_b_c_cw20.gt.1)   chem(i,k,j,p_pcg1_b_c_cw20)=0.0
                   if (p_pcg1_b_o_cw20.gt.1)   chem(i,k,j,p_pcg1_b_o_cw20)=0.0
                   if (p_opcg1_b_c_cw20.gt.1)  chem(i,k,j,p_opcg1_b_c_cw20)=0.0
                   if (p_opcg1_b_o_cw20.gt.1)  chem(i,k,j,p_opcg1_b_o_cw20)=0.0
                   if (p_pcg1_f_c_cw20.gt.1)   chem(i,k,j,p_pcg1_f_c_cw20)=0.0
                   if (p_pcg1_f_o_cw20.gt.1)   chem(i,k,j,p_pcg1_f_o_cw20)=0.0
                   if (p_opcg1_f_c_cw20.gt.1)  chem(i,k,j,p_opcg1_f_c_cw20)=0.0
                   if (p_opcg1_f_o_cw20.gt.1)  chem(i,k,j,p_opcg1_f_o_cw20)=0.0
                   if (p_ant1_c_cw20.gt.1)     chem(i,k,j,p_ant1_c_cw20)=0.0
                   if (p_biog1_c_cw20.gt.1)    chem(i,k,j,p_biog1_c_cw20)=0.0
                enddo
             enddo
          enddo
       endif
...
"ERROR: wet scavaging option requires mp_phys = CAMMGMP SCHEME to function.")
     ...
     CASE (CBMZ_MOSAIC_4BIN, CBMZ_MOSAIC_KPP, CBMZ_MOSAIC_8BIN, CBMZ_MOSAIC_4BIN_AQ, CBMZ_MOSAIC_8BIN_AQ, &
           ...
           SAPRC99_MOSAIC_8BIN_VBS2_KPP,SAPRC99_MOSAIC_20BIN_VBS2_AQ_KPP,SAPRC99_MOSAIC_20BIN_VBS2_KPP, & 
