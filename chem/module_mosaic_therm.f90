      ...
      subroutine aerchemistry( iclm, jclm, kclm_calcbgn, kclm_calcend,   &
                            dtchem_sngl, vbs_nbin,config_flags )
      ...
      do 200 m = 1, nsubareas
        ...
        do 100 k = kclm_aer_calcbgn, kclm_aer_calcend
          ...
          call mosaic( k, m, dtchem,vbs_nbin,config_flags)
...
200   continue		
      ...
      subroutine mosaic(k, m, dtchem,vbs_nbin, config_flags)
      ...
      if(iclm_aer .eq. iclm_debug .and.   &
        ...
        dumdum = 0.0
         ...
         if(1.eq.0)then
          ...
          call mosaic_dynamic_solver( dtchem,vbs_nbin, config_flags )
      ...
      subroutine mosaic_dynamic_solver( dtchem,vbs_nbin, config_flags )
      use module_configure, only:  grid_config_rec_type
...
500   continue
      ...
      if(mgas_aer_xfer .eq. mon)then
        ...
        call astem(dtchem,vbs_nbin,config_flags)
...
10    continue
	...
	if (kclno2 .ge. p1st) then
       ...
       if (kgly .ge. p1st) then
            gas(igly_g)   = rsub(kgly,k,m)*conv1a
        else
        gas(igly_g) = 0.0
        end if

        if (kiepox .ge. p1st) then
            gas(iiepox_g)   = rsub(kiepox,k,m)*conv1a
        else
        gas(iiepox_g) = 0.0
        end if


        if (ktetrol .ge. p1st) then
            gas(itetrol_g)   = rsub(ktetrol,k,m)*conv1a
        else
        gas(itetrol_g) = 0.0
        end if

        if (kcldtetrol .ge. p1st) then
            gas(icldtetrol_g)   = rsub(kcldtetrol,k,m)*conv1a
        else
        gas(icldtetrol_g) = 0.0
        end if
        ...
        l = lptr_tanv_aer(isize,itype,iphase)
        if (l .ge. p1st) then
            aer(itanv_a,jtotal,ibin)=rsub(l,k,m)*conv1a
        else
            aer(itanv_a,jtotal,ibin)=0.0
        end if


        l = lptr_sopnv_aer(isize,itype,iphase)
        if (l .ge. p1st) then
            aer(isopnv_a,jtotal,ibin)=rsub(l,k,m)*conv1a
        else
            aer(isopnv_a,jtotal,ibin)=0.0
        end if

        l = lptr_ternv_aer(isize,itype,iphase)
        if (l .ge. p1st) then
            aer(iternv_a,jtotal,ibin)=rsub(l,k,m)*conv1a
        else
            aer(iternv_a,jtotal,ibin)=0.0
        end if

        l = lptr_seqnv_aer(isize,itype,iphase)
        if (l .ge. p1st) then
            aer(iseqnv_a,jtotal,ibin)=rsub(l,k,m)*conv1a
        else
            aer(iseqnv_a,jtotal,ibin)=0.0
        end if


        l = lptr_sianv_aer(isize,itype,iphase)
        if (l .ge. p1st) then
            aer(isianv_a,jtotal,ibin)=rsub(l,k,m)*conv1a
        else
            aer(isianv_a,jtotal,ibin)=0.0
        end if
      ...
      l = lptr_gly_aer(isize,itype,iphase)
       if (l .ge. p1st) then
           aer(igly_a,jtotal,ibin)=rsub(l,k,m)*conv1a
       else
            aer(igly_a,jtotal,ibin)=0.0
       end if

       l = lptr_iepox_aer(isize,itype,iphase)
       if (l .ge. p1st) then
           aer(iiepox_a,jtotal,ibin)=rsub(l,k,m)*conv1a
       else
            aer(iiepox_a,jtotal,ibin)=0.0
       end if


       l = lptr_iepoxos_aer(isize,itype,iphase)
       if (l .ge. p1st) then
           aer(iiepoxos_a,jtotal,ibin)=rsub(l,k,m)*conv1a
       else
            aer(iiepoxos_a,jtotal,ibin)=0.0
       end if

       l = lptr_tetrol_aer(isize,itype,iphase)
       if (l .ge. p1st) then
           aer(itetrol_a,jtotal,ibin)=rsub(l,k,m)*conv1a
       else
            aer(itetrol_a,jtotal,ibin)=0.0
       end if

       l = lptr_cldiepoxos_aer(isize,itype,iphase)
       if (l .ge. p1st) then
           aer(icldiepoxos_a,jtotal,ibin)=rsub(l,k,m)*conv1a
       else
            aer(icldiepoxos_a,jtotal,ibin)=0.0
       end if

       l = lptr_cldtetrol_aer(isize,itype,iphase)
       if (l .ge. p1st) then
           aer(icldtetrol_a,jtotal,ibin)=rsub(l,k,m)*conv1a
       else
            aer(icldtetrol_a,jtotal,ibin)=0.0
       end if
...
10      continue
	...
	if (kclno2 .ge. p1st)   &
        ...
        if (kgly .ge. p1st)   &
            rsub(kgly,k,m)   = gas(igly_g)*conv1b
        


        if (kiepox .ge. p1st)   &
            rsub(kiepox,k,m)   = gas(iiepox_g)*conv1b

        if (ktetrol .ge. p1st)   &
            rsub(ktetrol,k,m)   = gas(itetrol_g)*conv1b

        if (kcldtetrol .ge. p1st)   &
            rsub(kcldtetrol,k,m)   = gas(icldtetrol_g)*conv1b
       ...
       l = lptr_gly_aer(isize,itype,iphase)
       if (l .ge. p1st) rsub(l,k,m) = aer(igly_a,jtotal,ibin)*conv1b
       l = lptr_iepox_aer(isize,itype,iphase)
       if (l .ge. p1st) rsub(l,k,m) = aer(iiepox_a,jtotal,ibin)*conv1b
       l = lptr_iepoxos_aer(isize,itype,iphase)
       if (l .ge. p1st) rsub(l,k,m) = aer(iiepoxos_a,jtotal,ibin)*conv1b
       l = lptr_tetrol_aer(isize,itype,iphase)
       if (l .ge. p1st) rsub(l,k,m) = aer(itetrol_a,jtotal,ibin)*conv1b
       l = lptr_cldiepoxos_aer(isize,itype,iphase)
       if (l .ge. p1st) rsub(l,k,m) = aer(icldiepoxos_a,jtotal,ibin)*conv1b
       l = lptr_cldtetrol_aer(isize,itype,iphase)
       if (l .ge. p1st) rsub(l,k,m) = aer(icldtetrol_a,jtotal,ibin)*conv1b
        l = lptr_tanv_aer(isize,itype,iphase)
        if (l .ge. p1st) rsub(l,k,m) = aer(itanv_a,jtotal,ibin)*conv1b
        l = lptr_sopnv_aer(isize,itype,iphase)
        if (l .ge. p1st) rsub(l,k,m) = aer(isopnv_a,jtotal,ibin)*conv1b
        l = lptr_ternv_aer(isize,itype,iphase)
        if (l .ge. p1st) rsub(l,k,m) = aer(iternv_a,jtotal,ibin)*conv1b
        l = lptr_seqnv_aer(isize,itype,iphase)
        if (l .ge. p1st) rsub(l,k,m) = aer(iseqnv_a,jtotal,ibin)*conv1b
        l = lptr_sianv_aer(isize,itype,iphase)
        if (l .ge. p1st) rsub(l,k,m) = aer(isianv_a,jtotal,ibin)*conv1b
       l = lptr_glysoa_r1_aer(isize,itype,iphase)
...
20      continue
       ...
       phvar1(k,m)=mc(jc_h,1)  
       phvar2(k,m)=mc(jc_h,2)
       phvar3(k,m)=mc(jc_h,3)
       phvar4(k,m)=mc(jc_h,4)
        ...
        iepoxcoatbin1(k,m)=coat(1)*1.0e7 
        iepoxcoatbin2(k,m)=coat(2)*1.0e7
        iepoxcoatbin3(k,m)=coat(3)*1.0e7
        iepoxcoatbin4(k,m)=coat(4)*1.0e7

       gammaiepoxaq(k,m)=gamma_iepox
       betaiepoxaq(k,m)=beta_iepox
...
10    call do_full_deliquescence(ibin)
      ...
      aer(itanv_a, jsolid,ibin) = aer(itanv_a,jtotal,ibin)
      aer(isopnv_a, jsolid,ibin) = aer(isopnv_a,jtotal,ibin)
      aer(iternv_a, jsolid,ibin) = aer(iternv_a,jtotal,ibin)
      aer(iseqnv_a, jsolid,ibin) = aer(iseqnv_a,jtotal,ibin)
      aer(isianv_a, jsolid,ibin) = aer(isianv_a,jtotal,ibin)
      ...
      aer(igly_a,jsolid,ibin)= aer(igly_a,jtotal,ibin)
      aer(iiepox_a,jsolid,ibin)= aer(iiepox_a,jtotal,ibin)
      aer(iiepoxos_a,jsolid,ibin)= aer(iiepoxos_a,jtotal,ibin)
      aer(itetrol_a,jsolid,ibin)= aer(itetrol_a,jtotal,ibin)
      aer(icldiepoxos_a,jsolid,ibin)= aer(icldiepoxos_a,jtotal,ibin)
      aer(icldtetrol_a,jsolid,ibin)= aer(icldtetrol_a,jtotal,ibin)
      ...
      aer(itanv_a, jliquid,ibin) = 0.0
      aer(isopnv_a, jliquid,ibin) = 0.0
      aer(iternv_a, jliquid,ibin) = 0.0
      aer(iseqnv_a, jliquid,ibin) = 0.0
      aer(isianv_a, jliquid,ibin) = 0.0
      ...
      aer(igly_a,jliquid,ibin)= 0.0
      aer(iiepox_a,jliquid,ibin)= 0.0
      aer(iiepoxos_a,jliquid,ibin)= 0.0
      aer(itetrol_a,jliquid,ibin)= 0.0
      aer(icldiepoxos_a,jliquid,ibin)= 0.0
      aer(icldtetrol_a,jliquid,ibin)= 0.0
...
10    return
      ...
      subroutine ASTEM(dtchem,vbs_nbin,config_flags)
      ...
      integer vbs_nbin(1)
      type(grid_config_rec_type), intent(in) :: config_flags
      integer start_svoc, Nsoa,nsemivol
      ...
      nsemivol   = ngas_ioa + ngas_soa

      if(config_flags%uq_aqsoa.eq.1) & 
      call aqsoa_gamma(dtchem,nsemivol)  
      call equilibrium(start_svoc,Nsoa,nsemivol,dtchem,vbs_nbin,config_flags)
...
2000  continue
      ...
      subroutine aqsoa_gamma(dtchem,nsemivol)
      use module_data_mosaic_asect
      use module_data_mosaic_other
      use module_state_description, only:  param_first_scalar
      implicit none
      ...
      real(kind=8), parameter :: mwiepox=118.0
      real(kind=8), parameter :: mwos=216.0
      real(kind=8), parameter :: mwtetrol=136.0
      real(kind=8), parameter :: mwgly =58.0
      real(kind=8), parameter :: tinys=1.d-15

      real(kind=8),intent(in):: dtchem


      integer, intent(in):: nsemivol
      real(kind=8) ::taqmass,volaq(nbin_a),aqdens,voltotal(nbin_a) 
      real(kind=8) :: aqsoa_con(10) 
      real(kind=8) :: iepoxtot,glytot 
      real(kind=8) :: iepoxgasinit,glygasinit
      real(kind=8) :: iepoxaer,iepoxosaer,tetrolaer,glyaer
      real(kind=8) ::  alft(nbin_a), conv1,conv2, dt1,tempcon1,tempcon2,parsum
      real(kind=8) :: a(10), b(10), f(10),frt, tempos,tempnh4
      real (kind=8) :: iepoxinit(nbin_a),osinit(nbin_a),tetrolinit(nbin_a),glyinit(nbin_a) 
      real (kind=8) :: os_begin(nbin_a)  
      real (kind=8) :: nh4_begin(nbin_a),gamma_iepox_bin(nbin_a) 
      real (kind=8) :: g_iepox_in 
      real (kind=8) :: delta_g_iepox 
      real (kind=8) :: cbar_iepox 
      real (kind=8) :: Satot 
      ...
      real(kind=8) :: g_iepoxdt,g_glydt
      real(kind=8) :: raq(nbin_a),rtotal(nbin_a) 
      real(kind=8) :: kaq,kso4
      character(len=200) :: str

      if (swdown_cell.gt.0.0) then 
      daylight=1
      else
      daylight=0
      endif
       ...
       conv1 = 1.d15/6.0221367d23     
       conv2 = 1.d0/conv1         

      do ibin=1,nbin_a
      os_begin(ibin)=max(aer(iiepoxos_a,jtotal,ibin),0.0d0)
      nh4_begin(ibin)=max(aer(inh4_a,jtotal,ibin),0.0d0)
      enddo


       g_iepox_in=gas(iiepox_g) 
      ...
      do ibin=1,nbin_a
        coat(ibin)=-999.0d0
        gamma_iepox_bin(ibin)=-999.0d0
        Satot=Satot+area_wet_a(ibin) 
      enddo

       cbar_iepox = (8.0 *8.314 * t_k / 3.141593 / 0.118)**0.5 * 100 
          ...
          dt1 = 1.0d0 
          niter = int(max(1.0d0,dtchem/dt1)) 
      ...
      iepoxtot=gas(iiepox_g)
      glytot=gas(igly_g)


      do ibin=1,nbin_a 
      iepoxtot=iepoxtot+aer(iiepox_a,jtotal,ibin) &
         +aer(iiepoxos_a,jtotal,ibin)+aer(itetrol_a,jtotal,ibin)
      glytot=glytot+aer(igly_a,jtotal,ibin)

      iepoxinit(ibin)=max(aer(iiepox_a,jtotal,ibin),0.0d0)
      osinit(ibin)=max(aer(iiepoxos_a,jtotal,ibin),0.0d0)
      tetrolinit(ibin)=max(aer(itetrol_a,jtotal,ibin),0.0d0)
      glyinit(ibin)=max(aer(igly_a,jtotal,ibin),0.0d0)
      enddo

      iepoxtot=max(iepoxtot,0.0d0)
      glytot=max(glytot,0.0d0)
      ...
      if(jaerosolstate(ibin) .ne. no_aerosol)then
       ...
       volaq(ibin)=aer(iso4_a,jliquid,ibin)*mw_aer_mac(iso4_a)/dens_aer_mac(iso4_a) &
      +aer(ino3_a,jliquid,ibin)*mw_aer_mac(ino3_a)/dens_aer_mac(ino3_a) &
      +aer(icl_a,jliquid,ibin)*mw_aer_mac(icl_a)/dens_aer_mac(icl_a)  &
      +aer(imsa_a,jliquid,ibin)*mw_aer_mac(imsa_a)/dens_aer_mac(imsa_a) &
      +aer(ico3_a,jliquid,ibin)*mw_aer_mac(ico3_a)/dens_aer_mac(ico3_a) &
      +aer(ica_a,jliquid,ibin)*mw_aer_mac(ica_a)/dens_aer_mac(ica_a) &
      +aer(ina_a,jliquid,ibin)*mw_aer_mac(ina_a)/dens_aer_mac(ina_a) &
      + aer(inh4_a,jliquid,ibin)*mw_aer_mac(inh4_a)/dens_aer_mac(inh4_a) & 
               ...
               +mc(jc_h,ibin)*water_a(ibin)*1.e9 & 
            ...
            +aerosol_water_inorg(jtotal,ibin)*1.e12/1.0 


              voltotal(ibin)=volaq(ibin) + &
         (aerosol_water(jtotal,ibin)-aerosol_water_inorg(jtotal,ibin))*1.e12/1.0  & 
               +aer(iiepox_a,jtotal,ibin)*mwiepox/dens_aer_mac(ioc_a) &
               +aer(iiepoxos_a,jtotal,ibin)*mwos/dens_aer_mac(ioc_a)  &
               +aer(itetrol_a,jtotal,ibin)*mwtetrol/dens_aer_mac(ioc_a) & 
               +aer(icldiepoxos_a,jtotal,ibin)*mwos/dens_aer_mac(ioc_a)  &
               +aer(icldtetrol_a,jtotal,ibin)*mwtetrol/dens_aer_mac(ioc_a) &
               +aer(igly_a,jtotal,ibin)*mwgly/dens_aer_mac(ioc_a)  
         do iv = ipcg1_b_c_g, nsemivol-2 
             voltotal(ibin)=voltotal(ibin)+aer(iv,jtotal,ibin)*250.0d0/1.0d0 
         enddo


          alft(ibin)=volaq(ibin)*1.d-15 
       ...
       if(volaq(ibin).gt.0.0d0) then
       raq(ibin)=(volaq(ibin)*1.0d-9*3.0d0/(4.0d0*3.141593*num_a(ibin)*1.0d6))**(1.0d0/3.0d0)
      
       rtotal(ibin)=(voltotal(ibin)*1.0d-9*3.0d0/(4.0d0*3.141593*num_a(ibin)*1.0d6))**(1.0d0/3.0d0)
       coat(ibin)=rtotal(ibin)-raq(ibin)
        endif
       ...
       if (alft(ibin).gt.1.D-35) then
       aqsoa_con(1) = gas(iiepox_g)*conv2 
       aqsoa_con(2) = gas(igly_g)*conv2   
       aqsoa_con(3) = aer(iiepox_a,jtotal,ibin)/max(alft(ibin),1.D-35)*1.D-12 
       aqsoa_con(4) =aer(iiepoxos_a,jtotal,ibin)/max(alft(ibin),1.D-35)*1.D-12
       aqsoa_con(5) = aer(itetrol_a,jtotal,ibin)/max(alft(ibin),1.D-35)*1.D-12
       aqsoa_con(6) = aer(igly_a,jtotal,ibin)/max(alft(ibin),1.D-35)*1.D-12
       aqsoa_con(7) = mc(jc_h,ibin)*water_a(ibin)/max(alft(ibin),1.D-35)*1.D-3 
       aqsoa_con(8) =  ma(ja_so4,ibin)*water_a(ibin)/max(alft(ibin),1.D-35)*1.D-3 
       aqsoa_con(9) = ma(ja_hso4,ibin)*water_a(ibin)/max(alft(ibin),1.D-35)*1.D-3 
       aqsoa_con(10) = mc(jc_nh4,ibin)*water_a(ibin)/max(alft(ibin),1.D-35)*1.D-3 
      ...
      call aqsoafn1(aqsoa_con,daylight,f,alft(ibin),ibin)





            parsum=aqsoa_con(3)+aqsoa_con(4)+aqsoa_con(5)
        ...
        if(abs(f(3)).gt.tinys) then
          ...
          if (f(3).lt.0.0) then 
             ...
             aqsoa_con(3) = max(aqsoa_con(3) + frt*f(3) * dt1, 0.0d0)
              ...
              aqsoa_con(3) = aqsoa_con(3) + f(3) * dt1 
          endif
          aer(iiepox_a,jtotal,ibin)=max(aqsoa_con(3)*max(alft(ibin),0.0d0)*1.D12,0.0d0) 
       endif
        ...
        if(abs(f(4)).gt.tinys) then
          if (f(4).lt.0.0) then 
             ...
             aqsoa_con(4) = max(aqsoa_con(4) + frt*f(4) * dt1,0.0d0)
              ...
              aqsoa_con(4) = aqsoa_con(4) + f(4) * dt1 
          endif
         aer(iiepoxos_a,jtotal,ibin)=max(aqsoa_con(4)*max(alft(ibin),0.0d0)*1.D12,0.0d0)
        endif

        if(abs(f(5)).gt.tinys) then
          if (f(5).lt.0.0) then 
             frt=MAX(MIN(aqsoa_con(5)/abs(f(5) * dt1),1.0d0),0.0d0)
             aqsoa_con(5) = max(aqsoa_con(5) + frt*f(5) * dt1,0.0d0)
          else
              aqsoa_con(5) = aqsoa_con(5) + f(5) * dt1 
          endif
          aer(itetrol_a,jtotal,ibin)=max(aqsoa_con(5)*max(alft(ibin),0.0d0)*1.D12,0.0d0)
        endif
         ...
         if(abs(f(6)).gt.tinys) then
           if (f(6).lt.0.0) then 
             frt=MAX(MIN(aqsoa_con(6)/abs(f(6) * dt1),1.0d0),0.0d0)
             aqsoa_con(6) = max(aqsoa_con(6) + frt*f(6) * dt1,0.0d0)
          else
              aqsoa_con(6) = aqsoa_con(6) + f(6) * dt1 
          endif
        aer(igly_a,jtotal,ibin)=max(aqsoa_con(6)*max(alft(ibin),0.0d0)*1.D12,0.0d0)
         endif
          ...
          if (daylight.eq.1) then 
             ...
             velgas = sqrt(8.0*8.31441d3*t_k/3.141593/58.0) 
             rrate = 1.0/4.0 * 3.0d-3 * velgas * area_wet_a(ibin)*100.0 


             if (rrate*dt1.ge.1.0) rrate = 1.0/dt1 
...
150    continue 
      ...
      tempos=aer(iiepoxos_a,jtotal,ibin)-os_begin(ibin)

      if (tempos.gt.0.0d0.and.os_begin(ibin).gt.0.0d0) then 
         if (aer(iso4_a,jtotal,ibin).ge.tempos) then
            
            aer(iso4_a,jtotal,ibin)=aer(iso4_a,jtotal,ibin)-tempos
            aer(inh4_a,jtotal,ibin)=aer(inh4_a,jtotal,ibin)-tempos

         else if (aer(iso4_a,jtotal,ibin).lt.tempos) then
            ...
            aer(iiepoxos_a,jtotal,ibin)= aer(iiepoxos_a,jtotal,ibin)- &
               (tempos-aer(iso4_a,jtotal,ibin)) 

            aer(inh4_a,jtotal,ibin)=aer(inh4_a,jtotal,ibin)-aer(iso4_a,jtotal,ibin) 
            aer(iso4_a,jtotal,ibin)=0.0d0 

         end if
      else 
      end if 

      if (aer(inh4_a,jtotal,ibin).lt.0.0d0) aer(inh4_a,jtotal,ibin)=0.0d0
      end do
         ...
         gas(iiepox_g) =iepoxtot
         gas(igly_g) = glytot
         glyaer=0.0d0
         iepoxaer=0.0d0
         iepoxosaer=0.0d0
         tetrolaer=0.0d0
         iepoxgasinit=gas(iiepox_g)
         glygasinit=gas(igly_g)

         do ibin=1,nbin_a
         iepoxaer=iepoxaer+aer(iiepox_a,jtotal,ibin)
         iepoxosaer=iepoxosaer+aer(iiepoxos_a,jtotal,ibin)
         tetrolaer=tetrolaer+aer(itetrol_a,jtotal,ibin)
         glyaer=glyaer+aer(igly_a,jtotal,ibin)
         enddo

        gas(iiepox_g) = gas(iiepox_g) - (iepoxaer+iepoxosaer+tetrolaer)
        gas(igly_g) = gas(igly_g)-glyaer


        if (gas(iiepox_g).lt.0.0) then
          gas(iiepox_g)=0.0d0

          do ibin=1,nbin_a
            if((iepoxaer+iepoxosaer+tetrolaer).gt.0.0d0) then
          aer(iiepox_a,jtotal,ibin)=iepoxgasinit*aer(iiepox_a,jtotal,ibin)/(iepoxaer+iepoxosaer+tetrolaer)
          aer(iiepoxos_a,jtotal,ibin)=iepoxgasinit*aer(iiepoxos_a,jtotal,ibin)/(iepoxaer+iepoxosaer+tetrolaer)
          aer(itetrol_a,jtotal,ibin)=iepoxgasinit*aer(itetrol_a,jtotal,ibin)/(iepoxaer+iepoxosaer+tetrolaer)
            else
          aer(iiepox_a,jtotal,ibin)=0.0d0
          aer(iiepoxos_a,jtotal,ibin)=0.0d0
          aer(itetrol_a,jtotal,ibin)=0.0d0
            endif
          enddo
         endif

        if (gas(igly_g).lt.0.0) then
         gas(igly_g)=0.0d0

          do ibin=1,nbin_a
           if(glyaer.gt.0.0d0) then
        aer(igly_a,jtotal,ibin)=glygasinit*aer(igly_a,jtotal,ibin)/glyaer 
           else
           aer(igly_a,jtotal,ibin)=0.0d0
           endif
          enddo
      ...
      tempnh4=aer(inh4_a,jtotal,ibin)-nh4_begin(ibin)

      if (tempnh4.gt.0.0d0.and.nh4_begin(ibin).gt.0.0d0) then 
         aer(iiepoxos_a,jtotal,ibin)= aer(iiepoxos_a,jtotal,ibin) + &
            tempnh4 * mw_aer_mac(inh4_a)/mwos 
      end if
      end do
       ...
       delta_g_iepox=(gas(iiepox_g)-g_iepox_in)/dtchem 
       if(Satot.gt.0.0d0.and.g_iepox_in.gt.0.0d0) then
       gamma_iepox=abs(delta_g_iepox/(-1.0/4.0*cbar_iepox*Satot*g_iepox_in))
       endif
      ...
      kaq = ( 5.0d-2 * aqsoa_con(7) + 2.0e-4 * &
          aqsoa_con(8) * aqsoa_con(7) + &
          7.3d-4 * aqsoa_con(9) + 1.7d-5 * &
          aqsoa_con(10) )

      kso4= 2.0d-4 *aqsoa_con(8) * aqsoa_con(7) + 7.3d-4 * aqsoa_con(9)
      ...
      beta_iepox=kso4/kaq
       ...
       end subroutine aqsoa_gamma


      subroutine aqsoafn1(aqsoa_con,daylight,f,alftcur,ib)
      use module_data_mosaic_other
      implicit none

      real(kind=8),intent(in) :: aqsoa_con(10),alftcur
      real(kind=8), intent(inout) :: f(10)
      real(kind=8) :: a(10),b(10)
      integer , intent(in) :: daylight
      real(kind=8) :: cbar_iepox, Dg_iepox, alpha_iepox, H_iepox
      real(kind=8) :: cbar_gly, Dg_gly, alpha_gly, H_gly
      real(kind=8) :: freepath_iepox, kn_iepox, fkn_iepox
      real(kind=8) :: freepath_gly, kn_gly, fkn_gly
      ...
      cbar_iepox = (8.0 *8.314 * t_k / 3.141593 / 0.118)**0.5 * 100 
      Dg_iepox = 1.9 * 0.118**(-2.0/3.0) / 100.0 
      alpha_iepox = 0.1  

      freepath_iepox=3.*Dg_iepox/cbar_iepox
      kn_iepox=2.*freepath_iepox/dp_wet_a(ib)
      fkn_iepox=fuchs_sutugin(kn_iepox,alpha_iepox)


      H_iepox =  3.0e7 

      cbar_gly = (8.0 *8.314 * t_k / 3.141593 / 0.058)**0.5 * 100 
      Dg_gly = 1.9 * 0.058**(-2.0/3.0) / 100.0 
      H_gly = 2.7e7 
      beta = 0.4
      alpha_gly = 0.023

      freepath_gly=3.*Dg_gly/cbar_gly
      kn_gly=2.*freepath_gly/dp_wet_a(ib)
      fkn_gly=fuchs_sutugin(kn_gly,alpha_gly)
      ...
      kmt = ( (dp_wet_a(ib)/2.0)**2.0 / (3.0 *Dg_iepox) + 4.0  &
        * (dp_wet_a(ib)/2.0)/ (3.0 * cbar_iepox * alpha_iepox) )**(-1.0) 


      
      a(1) = aqsoa_con(is2+1) / H_iepox * 2.46e19 *298.0/t_k*p_atm* alftcur &
          * kmt
      b(1) = alftcur * kmt
      ...
      if ((aqsoa_con(is2+6)+aqsoa_con(is2+7)).ge.aqsoa_con(is2+1)) then 
      ka_os = ( 5.0e-2 * aqsoa_con(is2+5) + 2.0e-4 * &
          aqsoa_con(is2+6) * aqsoa_con(is2+5) + &
          7.3e-4 * aqsoa_con(is2+7) + 1.7e-5 * &
          aqsoa_con(is2+8) ) * beta
      else
      ka_os=0.0
      endif

      ka_tetrol = ( 5.0e-2 * aqsoa_con(is2+5) + 2.0e-4 * &
          aqsoa_con(is2+6) * aqsoa_con(is2+5) + &
          7.3e-4 * aqsoa_con(is2+7) + 1.7e-5 * &
          aqsoa_con(is2+8) ) * (1.0-beta)


      
      a(is2+1) = kmt * aqsoa_con(1) * 1000.0 / 6.023e23
      b(is2+1) = (kmt * 1000.0 / H_iepox / t_k / 82.06 + &
          ka_os + ka_tetrol )
      ...
      if (daylight.eq.0) then 
         ...
         kmt = ( (dp_wet_a(ib)/2.0)**2.0 / (3.0 * Dg_gly)  &
              + 4.0 *(dp_wet_a(ib)/2.0) &
             / (3.0 * cbar_gly * alpha_gly) )**(-1.0)

         
         a(2) = aqsoa_con(is2+4) / H_gly * 2.46e19 * 298.0/t_k*p_atm*alftcur &
             * kmt
         b(2) = alftcur * kmt
         f(2) = a(2) - b(2) * aqsoa_con(2)

         
         a(is2+4) = kmt * aqsoa_con(2) * 1000.0 / 6.023e23
         b(is2+4) = kmt * 1000.0 / H_gly / t_k / 82.06
         f(is2+4) = a(is2+4) - b(is2+4) * aqsoa_con(is2+4)
      ...
      subroutine aqsoafn2(aqsoa_con,daylight,f,alftcur,raqcur,rtotalcur,coatcur,ib)
      use module_data_mosaic_other

      implicit none

      real(kind=8), parameter :: airmolecrad=0.5e-10 
      real(kind=8),intent(in) :: aqsoa_con(10),alftcur,coatcur,raqcur,rtotalcur
      real(kind=8), intent(inout) :: f(10)
      real(kind=8) :: a(10),b(10)
      real(kind=8) :: gamma_tot_iepox,kaq,tauaq,invtauorg,gasterm
      integer , intent(in) :: daylight
      real(kind=8) :: cbar_iepox, Dg_iepox, alpha_iepox, H_iepox
      real(kind=8) :: cbar_gly, Dg_gly, alpha_gly, H_gly
      real(kind=8) :: freepath_iepox, kn_iepox, fkn_iepox
      ...
      if (rh_pc.le.30) then
      etaorg=7.0e5
      elseif(rh_pc.gt.30.and.rh_pc.le.50) then
      etaorg=3.0e4
      elseif(rh_pc.gt.50.and.rh_pc.le.60) then
      etaorg=2.0e4
      elseif(rh_pc.gt.60.and.rh_pc.le.70) then
      etaorg=3.0e3
      elseif(rh_pc.gt.70.and.rh_pc.le.80) then
      etaorg=3.0d0
      elseif(rh_pc.gt.80) then
      etaorg=0.3d0
      endif
      ...
      Dg_iepox = 1.9 * 0.118**(-2.0/3.0) / 100.0 
      alpha_iepox = 0.1  
      Dorg=1.38e-23*t_k/(6.0d0*3.141593*airmolecrad*etaorg)*1.0e4 
      ...
      kaq = ( 5.0e-2 * aqsoa_con(is2+5) + 2.0e-4 * &
          aqsoa_con(is2+6) * aqsoa_con(is2+5) + &
          7.3e-4 * aqsoa_con(is2+7) + 1.7e-5 * &
          aqsoa_con(is2+8) )

      tauaq=4.0d0*alftcur*0.08206*t_k*H_iepox*kaq/ &
            (cbar_iepox*area_wet_a(ib))

      if (coatcur.gt.0.0d0.and.raqcur.gt.0.0d0) then
       invtauorg=cbar_iepox*coatcur*1.0d0/(4.0d0*0.08206*t_k*Horg*Dorg)*rtotalcur/raqcur
      else
      invtauorg=0.0d0
      endif
      ...
      gasterm=dp_wet_a(ib)/2.0d0*cbar_iepox/(4.0d0*Dg_iepox)


       if(invtauorg.gt.0.0d0.and.tauaq.gt.0.0d0) then
       gamma_tot_iepox=(gasterm+1.0d0/alpha_iepox+1.0d0/tauaq+invtauorg)**(-1.0d0)
      elseif(tauaq.gt.0.0d0) then
      ...
      f(1)= -1.0d0/4.0d0*gamma_tot_iepox*cbar_iepox*area_wet_a(ib)*aqsoa_con(1)

      



      f(is2+1) =1.0d0/4.0d0*gamma_tot_iepox*cbar_iepox*area_wet_a(ib)* &
      ...
      if (daylight.eq.0) then 
        ...
        subroutine  equilibrium(start_ind,N,nsemivol,dtchem,vbs_nbin,config_flags)
     ...
     use module_configure, only:  grid_config_rec_type
        ...
        real(kind=8),intent(in)  :: dtchem
        integer, intent(in) :: start_ind, N, nsemivol

        integer, parameter :: itermax=8000
        ...
        real(kind=8) :: Paer(nsemivol)
        real(kind=8) :: Xp(nsemivol),Coa 
        integer :: i

        jp=jtotal
        iter=0
         irrev=config_flags%uq_par
         soaphotol=config_flags%uq_phot
...
'Error in mapping fresh and primary species')
        endif
...
120       continue
           enddo 
              ...
              do iv = start_ind, (start_ind + N - 1)
                  do ibin = 1, nbin_a
               Coa=Coa+aer(iv,jtotal,ibin)
                 enddo
               enddo

              do iv = start_ind, (start_ind + N - 1)
                 if(Coa.gt.1d-10) then
                Xp(iv)=1.0/(1.0+sat_soa(iv)/Coa)
                 endif
              enddo
         ...
         if (soaphotol.eq.1) call soaphotolysis (dtchem,flagsoap,start_ind,N)
         if (irrev.eq.1) call irreverse(dtchem,flagsoap,start_ind,N)


       end subroutine equilibrium

       subroutine soaphotolysis(dtchem,flagsoap,start_ind,N)
        implicit none
         real(kind=8):: dtchem
         real(kind=8):: isop_phot,terp_phot,sesq_phot
         integer,intent(in) :: flagsoap(ngas_soa),start_ind,N
         integer ibin,iv

          isop_phot=phno2_cell*0.015d0/60.0d0  
          terp_phot=phno2_cell*0.008d0/60.0d0
          sesq_phot=phno2_cell*0.006d0/60.0d0
          ...
          do iv = start_ind, (start_ind + N - 1)
             ...
             do ibin=1,nbin_a
              if ((iv.ge.84.and.iv.le.87)) then  
               aer(iv,jtotal,ibin)=aer(iv,jtotal,ibin)  &
                       *(0.08+0.92*exp(-1.0d0*isop_phot*dtchem))
              endif

              if (iv.eq.81.or.iv.eq.82) then  
               aer(iv,jtotal,ibin)=aer(iv,jtotal,ibin)  &
                       *(0.08+0.92*exp(-1.0d0*isop_phot*dtchem))
              endif

              if (iv.ge.88.and.iv.le.91) then
               aer(iv,jtotal,ibin)=aer(iv,jtotal,ibin)  &
                       *(0.32+0.68*exp(-1.0d0*terp_phot*dtchem))
              endif

              if (iv.ge.76.and.iv.le.79) then
               aer(iv,jtotal,ibin)=aer(iv,jtotal,ibin)  &
                       *(0.27+0.73*exp(-1.0d0*sesq_phot*dtchem))
              endif
       ...
       end subroutine soaphotolysis


      subroutine irreverse(dtchem,flagsoap,start_ind,N)



        implicit none
         real(kind=8):: dtchem,timein
         integer,intent(in) :: flagsoap(ngas_soa),start_ind,N
         integer ibin,iv
         ...
         timein=300.0d0 
          ...
          do iv = start_ind, (start_ind + N - 1)
             ...
             if (iv.ge.35.and.iv.le.37) then 
               aer(isianv_a,jtotal,ibin)=aer(isianv_a,jtotal,ibin) &
              +     aer(iv,jtotal,ibin)*min(1.0d0,dtchem/timein)
               aer(iv,jtotal,ibin) = aer(iv,jtotal,ibin)  &
               - aer(iv,jtotal,ibin)*min(1.0d0,dtchem/timein)
              endif

             if (iv.ge.67.and.iv.le.71) then 
               aer(isianv_a,jtotal,ibin)=aer(isianv_a,jtotal,ibin) &
              +     aer(iv,jtotal,ibin)*min(1.0d0,dtchem/timein)
               aer(iv,jtotal,ibin) = aer(iv,jtotal,ibin)  &
               - aer(iv,jtotal,ibin)*min(1.0d0,dtchem/timein)
             ...
             if (iv.ge.76.and.iv.le.79) then 
               aer(iseqnv_a,jtotal,ibin)=aer(iseqnv_a,jtotal,ibin) &
              +     aer(iv,jtotal,ibin)*min(1.0d0,dtchem/timein)
               aer(iv,jtotal,ibin) = aer(iv,jtotal,ibin)  &
               - aer(iv,jtotal,ibin)*min(1.0d0,dtchem/timein)
              endif
             ...
             if ((iv.ge.84.and.iv.le.87)) then 
               aer(isopnv_a,jtotal,ibin)=aer(isopnv_a,jtotal,ibin) &
              +     aer(iv,jtotal,ibin)*min(1.0d0,dtchem/timein)
               aer(iv,jtotal,ibin) = aer(iv,jtotal,ibin)  &
               - aer(iv,jtotal,ibin)*min(1.0d0,dtchem/timein)
              endif
             ...
             if (iv.ge.88.and.iv.le.91) then 
               aer(iternv_a,jtotal,ibin)=aer(iternv_a,jtotal,ibin) &
              +     aer(iv,jtotal,ibin)*min(1.0d0,dtchem/timein)
               aer(iv,jtotal,ibin) = aer(iv,jtotal,ibin)  &
               - aer(iv,jtotal,ibin)*min(1.0d0,dtchem/timein)
              endif
      ...
      implicit none
       ...
       integer,intent(in) :: N
        ...
        implicit none
        real(kind=8),  parameter :: xtol = 1.0e-5
...
100   if(iupdate_phase_state .eq. mYES)then
      ...
      mw_vol(igly_g)=58.0
      mw_vol(itetrol_g)=136.0
      mw_vol(iiepox_g)=118.0
      mw_vol(icldtetrol_g)=136.0
...
11    continue
      ...
      vol_dry_a(ibin) = vol_dry_a(ibin)*1.e-15				
        ...
        comp_a(join) = aer(ioin_a,jtotal,ibin)*mw_comp_a(je)*1.e-15	
         ...
         comp_a(jgly)= aer(igly_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15 
         comp_a(jiepox)= aer(iiepox_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15 
         comp_a(jiepoxos)= aer(iiepoxos_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15 
         comp_a(jtetrol)= aer(itetrol_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15 
         comp_a(jcldiepoxos)=aer(icldiepoxos_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15 
         comp_a(jcldtetrol)=aer(icldtetrol_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15 
        ...
        comp_a(jtanv)  = aer(itanv_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15      
        comp_a(jsopnv)  = aer(isopnv_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15      
        comp_a(jternv)  = aer(iternv_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15      
        comp_a(jseqnv)  = aer(iseqnv_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15      
        comp_a(jsianv)  = aer(isianv_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15      
...
10      if(jp .eq. jsolid)then
	...
	integer je					
      ...
      aer_name(ibc_a)  = 'bc '
      aer_name(ioin_a) = 'oin'
      aer_name(itanv_a)  = 'tanv'
      aer_name(isopnv_a)  = 'sopnv'
      aer_name(iternv_a)  = 'ternv'
      aer_name(iseqnv_a)  = 'seqnv'
      aer_name(isianv_a)  = 'sianv'
      ...
      aer_name(igly_a)="gly"
      aer_name(iiepox_a)="iepox"
      aer_name(iiepoxos_a)="iepoxos"
      aer_name(itetrol_a)="tetrol"
      aer_name(icldiepoxos_a)="cldiepoxos"
      aer_name(icldtetrol_a)="cldtetrol"
      ...
      gas_name(igly_g)="gly"
      gas_name(itetrol_g)="tetrol"
      gas_name(icldtetrol_g)="cldtetrol"
      gas_name(iiepox_g)="iepox"
      ...
      dens_comp_a(jtanv)      = 1.0
      dens_comp_a(jsopnv)      = 1.0
      dens_comp_a(jternv)      = 1.0
      dens_comp_a(jseqnv)      = 1.0
      dens_comp_a(jsianv)      = 1.0
      ...
      dens_comp_a(jiepox)=1.0
      dens_comp_a(jgly)=1.0
      dens_comp_a(jiepoxos)=1.0
      dens_comp_a(jtetrol)=1.0
      dens_comp_a(jcldiepoxos)=1.0
      dens_comp_a(jcldtetrol)=1.0
      ...
      mw_aer_mac(itanv_a)  = 250.0
      mw_aer_mac(isopnv_a)  = 250.0
      mw_aer_mac(iternv_a)  = 250.0
      mw_aer_mac(iseqnv_a)  = 250.0
      mw_aer_mac(isianv_a)  = 250.0
      ...
      mw_aer_mac(igly_a) = 58.0
      mw_aer_mac(iiepox_a) = 118.0
      mw_aer_mac(iiepoxos_a) = 216.0
      mw_aer_mac(itetrol_a) = 136.0
      mw_aer_mac(icldiepoxos_a) = 216.0
      mw_aer_mac(icldtetrol_a) = 136.0
      ...
      mw_comp_a(join)    = 1.0
      mw_comp_a(jh2o)    = 18.0
      mw_comp_a(jtanv)     = 250.0
      mw_comp_a(jsopnv)     = 250.0
      mw_comp_a(jternv)     = 250.0
      mw_comp_a(jseqnv)     = 250.0
      mw_comp_a(jsianv)     = 250.0
      ...
      mw_comp_a(jgly)=58.0
      mw_comp_a(jiepox)=118.0
      mw_comp_a(jiepoxos)=216.0
      mw_comp_a(jtetrol)=136.0
      mw_comp_a(jcldiepoxos)=216.0
      mw_comp_a(jcldtetrol)=136.0
      ...
      dens_aer_mac(itanv_a)  = 1.0
      dens_aer_mac(isopnv_a)  = 1.0
      dens_aer_mac(iternv_a)  = 1.0
      dens_aer_mac(iseqnv_a)  = 1.0
      dens_aer_mac(isianv_a)  = 1.0
      ...
      dens_aer_mac(igly_a)=1.0
      dens_aer_mac(iiepox_a)=1.0
      dens_aer_mac(iiepoxos_a)=1.0
      dens_aer_mac(itetrol_a)=1.0
      dens_aer_mac(icldiepoxos_a)=1.0
      dens_aer_mac(icldtetrol_a)=1.0
      ...
      partial_molar_vol(iiepox_g)=118.0
      partial_molar_vol(itetrol_g)=136.0
      partial_molar_vol(icldtetrol_g)=136.0
      partial_molar_vol(igly_g)=58.0
      ...
      po_soa(ibiog2_c_g) = fn_po(9.91d-6, 83.0d0, T_K) 
      ...
      if (vbs_nbin(1).eq.5) then
      po_soa(ipcg7_b_c_g) = fn_po(9.91d0, 64.0d0, T_K) 
      po_soa(ipcg7_f_c_g) = fn_po(9.91d0, 64.0d0, T_K) 

      po_soa(ipcg1_b_c_g) = fn_po(9.91d-9, 83.0d0, T_K) 
      po_soa(ipcg1_f_c_g) = fn_po(9.91d-9, 83.0d0, T_K) 
      ...
      po_soa(iopcg1_b_o_g) = fn_po(9.91d-6, 100.0d0, T_K) 
      po_soa(iopcg2_b_o_g) = fn_po(9.91d-5, 94.0d0, T_K) 
      po_soa(iopcg3_b_o_g) = fn_po(9.91d-4, 88.0d0, T_K) 
      po_soa(iopcg4_b_o_g) = fn_po(9.91d-3, 82.0d0, T_K) 

      po_soa(iopcg5_b_o_g) = fn_po(9.91d-6, 100.0d0, T_K) 
      po_soa(iopcg6_b_o_g) = fn_po(9.91d-5, 94.0d0, T_K) 
      po_soa(iopcg7_b_o_g) = fn_po(9.91d-4, 88.0d0, T_K) 
      po_soa(iopcg8_b_o_g) = fn_po(9.91d-3, 82.0d0, T_K) 
      ...
      po_soa(ibiog2_c_g) = fn_po(9.91d-6, 100.0d0, T_K) 
      po_soa(ibiog3_c_g) = fn_po(9.91d-5, 94.0d0, T_K) 
      po_soa(ibiog4_c_g) = fn_po(9.91d-4, 88.0d0, T_K) 

      po_soa(ibiog1_o_g) = fn_po(9.91d-7, 106.0d0, T_K) 
      po_soa(ibiog2_o_g) = fn_po(9.91d-6, 100.0d0, T_K) 
      po_soa(ibiog3_o_g) = fn_po(9.91d-5, 94.0d0, T_K) 
      po_soa(ibiog4_o_g) = fn_po(9.91d-4, 88.0d0, T_K) 

      po_soa(iant2_o_g) = fn_po(6.937d-6, 100.0d0, T_K) 
      po_soa(iant3_o_g) = fn_po(9.91d-5, 94.0d0, T_K) 

      po_soa(itetrol_g) = fn_po(1.82d-4, 94.0d0, T_K) 
                                                      

      po_soa(icldtetrol_g) = fn_po(1.82d-4, 94.0d0, T_K)
      endif
      ...
      real(kind=8) function aerosol_water_inorg(jp,ibin) 
      implicit none
      ...
      real(kind=8) dum, tmpa, kappa_poa, kappa_bc, kappa_soa, kappa_oin 
      ...
      aerosol_water_inorg = dum*1.e-9  
      return
      end function aerosol_water_inorg
      ...
      aer_soa = aer(iopcg2_b_o_a,jtotal,ibin) + &
                ...
                aer(itanv_a,jtotal,ibin) + &
                aer(isopnv_a,jtotal,ibin) + &
                aer(iternv_a,jtotal,ibin) + &
                aer(iseqnv_a,jtotal,ibin) + &
               aer(isianv_a,jtotal,ibin) + &
               aer(iiepox_a,jtotal,ibin) + &
               aer(iiepoxos_a,jtotal,ibin) + &
               aer(itetrol_a,jtotal,ibin)+ &
               aer(icldiepoxos_a,jtotal,ibin) + &
               aer(icldtetrol_a,jtotal,ibin) 
      ...
      dum = dum + &
            1e-3*((aer(ioc_a,jtotal,ibin)*mw_aer_mac(ioc_a)/dens_aer_mac(ioc_a))*kappa_poa*aH2O_a(ibin)/(1.0-aH2O_a(ibin)) + &
             (aer(ipcg1_b_c_a,jtotal,ibin)*mw_aer_mac(ipcg1_b_c_a)/dens_aer_mac(ipcg1_b_c_a))*kappa_poa*aH2O_a(ibin)/(1.0-aH2O_a(ibin))+ &
