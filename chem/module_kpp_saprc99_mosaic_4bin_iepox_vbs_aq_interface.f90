...
MODULE module_kpp_saprc99_mosaic_4bin_iepox_vbs_aq_interf 
  ...
  USE saprc99_mosaic_4bin_iepox_vbs_aq_UpdateRconstWRF
  USE saprc99_mosaic_4bin_iepox_vbs_aq_Integrator
...
SUBROUTINE  saprc99_mosaic_4bin_iepox_vbs_aq_interface( &
   ...
   RCNTRL(3) = 0.01_dp * TIME_END
    ...
    var(ind_ISOPOO) = conv  * REAL( MAX(chem(i,k,j,P_isopoo),0.), KIND=dp)  
    var(ind_ISOP2O2) = conv  * REAL( MAX(chem(i,k,j,P_isop2o2),0.), KIND=dp)  
    var(ind_ISOPOOH) = conv  * REAL( MAX(chem(i,k,j,P_isopooh),0.), KIND=dp)  
    var(ind_IEPOX) = conv  * REAL( MAX(chem(i,k,j,P_iepox),0.), KIND=dp)  
    var(ind_TETROL) = conv  * REAL( MAX(chem(i,k,j,P_tetrol),0.), KIND=dp)  
    ...
    var(ind_ANT2_c) = conv  * REAL( MAX(chem(i,k,j,P_ant2_c),0.), KIND=dp)  
    var(ind_ANT3_c) = conv  * REAL( MAX(chem(i,k,j,P_ant3_c),0.), KIND=dp)  
    var(ind_ANT4_c) = conv  * REAL( MAX(chem(i,k,j,P_ant4_c),0.), KIND=dp)  
    var(ind_ANT2_o) = conv  * REAL( MAX(chem(i,k,j,P_ant2_o),0.), KIND=dp)  
    var(ind_ANT3_o) = conv  * REAL( MAX(chem(i,k,j,P_ant3_o),0.), KIND=dp)  
    var(ind_BIOG1_c) = conv  * REAL( MAX(chem(i,k,j,P_biog1_c),0.), KIND=dp)  
    var(ind_BIOG2_c) = conv  * REAL( MAX(chem(i,k,j,P_biog2_c),0.), KIND=dp)  
    var(ind_BIOG3_c) = conv  * REAL( MAX(chem(i,k,j,P_biog3_c),0.), KIND=dp)  
    var(ind_BIOG4_c) = conv  * REAL( MAX(chem(i,k,j,P_biog4_c),0.), KIND=dp)  
    var(ind_BIOG1_o) = conv  * REAL( MAX(chem(i,k,j,P_biog1_o),0.), KIND=dp)  
    var(ind_BIOG2_o) = conv  * REAL( MAX(chem(i,k,j,P_biog2_o),0.), KIND=dp)  
    var(ind_BIOG3_o) = conv  * REAL( MAX(chem(i,k,j,P_biog3_o),0.), KIND=dp)  
    var(ind_BIOG4_o) = conv  * REAL( MAX(chem(i,k,j,P_biog4_o),0.), KIND=dp)  
    var(ind_PSD1) = conv  * REAL( MAX(chem(i,k,j,P_psd1),0.), KIND=dp)  
    var(ind_PCG7_F_C) = conv  * REAL( MAX(chem(i,k,j,P_pcg7_f_c),0.), KIND=dp)  
    var(ind_OPCG2_F_O) = conv  * REAL( MAX(chem(i,k,j,P_opcg2_f_o),0.), KIND=dp)  
    var(ind_OPCG3_F_O) = conv  * REAL( MAX(chem(i,k,j,P_opcg3_f_o),0.), KIND=dp)  
    var(ind_OPCG4_F_O) = conv  * REAL( MAX(chem(i,k,j,P_opcg4_f_o),0.), KIND=dp)  
    var(ind_OPCG5_F_O) = conv  * REAL( MAX(chem(i,k,j,P_opcg5_f_o),0.), KIND=dp)  
    var(ind_OPCG6_F_O) = conv  * REAL( MAX(chem(i,k,j,P_opcg6_f_o),0.), KIND=dp)  
    var(ind_PCG7_B_C) = conv  * REAL( MAX(chem(i,k,j,P_pcg7_b_c),0.), KIND=dp)  
    var(ind_PCG7_B_O) = conv  * REAL( MAX(chem(i,k,j,P_pcg7_b_o),0.), KIND=dp)  
    var(ind_OPCG1_B_O) = conv  * REAL( MAX(chem(i,k,j,P_opcg1_b_o),0.), KIND=dp)  
    var(ind_OPCG2_B_O) = conv  * REAL( MAX(chem(i,k,j,P_opcg2_b_o),0.), KIND=dp)  
    var(ind_OPCG3_B_O) = conv  * REAL( MAX(chem(i,k,j,P_opcg3_b_o),0.), KIND=dp)  
    var(ind_OPCG4_B_O) = conv  * REAL( MAX(chem(i,k,j,P_opcg4_b_o),0.), KIND=dp)  
    var(ind_OPCG5_B_O) = conv  * REAL( MAX(chem(i,k,j,P_opcg5_b_o),0.), KIND=dp)  
    var(ind_OPCG6_B_O) = conv  * REAL( MAX(chem(i,k,j,P_opcg6_b_o),0.), KIND=dp)  
    var(ind_OPCG7_B_O) = conv  * REAL( MAX(chem(i,k,j,P_opcg7_b_o),0.), KIND=dp)  
   ...
   CALL saprc99_mosaic_4bin_iepox_vbs_aq_Update_Rconst(  &
  ...
  CALL saprc99_mosaic_4bin_iepox_vbs_aq_INTEGRATE(TIME_START, TIME_END, &  
    ...
    chem(i,k,j,P_isopoo) = MAX ( REAL (oconv * var(ind_ISOPOO), KIND=sp), 0.)  
    chem(i,k,j,P_isop2o2) = MAX ( REAL (oconv * var(ind_ISOP2O2), KIND=sp), 0.)  
    chem(i,k,j,P_isopooh) = MAX ( REAL (oconv * var(ind_ISOPOOH), KIND=sp), 0.)  
    chem(i,k,j,P_iepox) = MAX ( REAL (oconv * var(ind_IEPOX), KIND=sp), 0.)  
    chem(i,k,j,P_tetrol) = MAX ( REAL (oconv * var(ind_TETROL), KIND=sp), 0.)  
   ...
   tbu_o(i,k,j) = MAX (REAL (oconv * var(ind_TBU_O) , KIND=sp),0.) 
    ...
    chem(i,k,j,P_ant2_c) = MAX ( REAL (oconv * var(ind_ANT2_c), KIND=sp), 0.)  
    chem(i,k,j,P_ant3_c) = MAX ( REAL (oconv * var(ind_ANT3_c), KIND=sp), 0.)  
    chem(i,k,j,P_ant4_c) = MAX ( REAL (oconv * var(ind_ANT4_c), KIND=sp), 0.)  
    chem(i,k,j,P_ant2_o) = MAX ( REAL (oconv * var(ind_ANT2_o), KIND=sp), 0.)  
    chem(i,k,j,P_ant3_o) = MAX ( REAL (oconv * var(ind_ANT3_o), KIND=sp), 0.)  
    chem(i,k,j,P_biog1_c) = MAX ( REAL (oconv * var(ind_BIOG1_c), KIND=sp), 0.)  
    chem(i,k,j,P_biog2_c) = MAX ( REAL (oconv * var(ind_BIOG2_c), KIND=sp), 0.)  
    chem(i,k,j,P_biog3_c) = MAX ( REAL (oconv * var(ind_BIOG3_c), KIND=sp), 0.)  
    chem(i,k,j,P_biog4_c) = MAX ( REAL (oconv * var(ind_BIOG4_c), KIND=sp), 0.)  
    chem(i,k,j,P_biog1_o) = MAX ( REAL (oconv * var(ind_BIOG1_o), KIND=sp), 0.)  
    chem(i,k,j,P_biog2_o) = MAX ( REAL (oconv * var(ind_BIOG2_o), KIND=sp), 0.)  
    chem(i,k,j,P_biog3_o) = MAX ( REAL (oconv * var(ind_BIOG3_o), KIND=sp), 0.)  
    chem(i,k,j,P_biog4_o) = MAX ( REAL (oconv * var(ind_BIOG4_o), KIND=sp), 0.)  
    chem(i,k,j,P_psd1) = MAX ( REAL (oconv * var(ind_PSD1), KIND=sp), 0.)  
    chem(i,k,j,P_pcg7_f_c) = MAX ( REAL (oconv * var(ind_PCG7_F_C), KIND=sp), 0.)  
    chem(i,k,j,P_opcg2_f_o) = MAX ( REAL (oconv * var(ind_OPCG2_F_O), KIND=sp), 0.)  
    chem(i,k,j,P_opcg3_f_o) = MAX ( REAL (oconv * var(ind_OPCG3_F_O), KIND=sp), 0.)  
    chem(i,k,j,P_opcg4_f_o) = MAX ( REAL (oconv * var(ind_OPCG4_F_O), KIND=sp), 0.)  
    chem(i,k,j,P_opcg5_f_o) = MAX ( REAL (oconv * var(ind_OPCG5_F_O), KIND=sp), 0.)  
    chem(i,k,j,P_opcg6_f_o) = MAX ( REAL (oconv * var(ind_OPCG6_F_O), KIND=sp), 0.)  
    chem(i,k,j,P_pcg7_b_c) = MAX ( REAL (oconv * var(ind_PCG7_B_C), KIND=sp), 0.)  
    chem(i,k,j,P_pcg7_b_o) = MAX ( REAL (oconv * var(ind_PCG7_B_O), KIND=sp), 0.)  
    chem(i,k,j,P_opcg1_b_o) = MAX ( REAL (oconv * var(ind_OPCG1_B_O), KIND=sp), 0.)  
    chem(i,k,j,P_opcg2_b_o) = MAX ( REAL (oconv * var(ind_OPCG2_B_O), KIND=sp), 0.)  
    chem(i,k,j,P_opcg3_b_o) = MAX ( REAL (oconv * var(ind_OPCG3_B_O), KIND=sp), 0.)  
    chem(i,k,j,P_opcg4_b_o) = MAX ( REAL (oconv * var(ind_OPCG4_B_O), KIND=sp), 0.)  
    chem(i,k,j,P_opcg5_b_o) = MAX ( REAL (oconv * var(ind_OPCG5_B_O), KIND=sp), 0.)  
    chem(i,k,j,P_opcg6_b_o) = MAX ( REAL (oconv * var(ind_OPCG6_B_O), KIND=sp), 0.)  
    chem(i,k,j,P_opcg7_b_o) = MAX ( REAL (oconv * var(ind_OPCG7_B_O), KIND=sp), 0.)  
...
END SUBROUTINE  saprc99_mosaic_4bin_iepox_vbs_aq_interface


END MODULE module_kpp_saprc99_mosaic_4bin_iepox_vbs_aq_interf 
